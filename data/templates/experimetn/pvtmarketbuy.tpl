...
{/section}
{if $message != ""}<br /><br />{/if}
<table class="invisible" cellspacing=0 cellpadding=0 width=93%><tr><td width=10%>&nbsp;</td><td class="acenter" style="vertical-align:top"><a href="?standards{$authstr}">Recruitment</a> | <b>Mercenaries</b> | <a href="?pvtmarketsell{$authstr}">Disband Troops</a> | <a href="?standards{$authstr}">Standards</a></td><td style="vertical-align:top" width=10% align=right><table cellspacing="0" cellpadding="0"><tr><td class="helpbutton"><a href="?guide&section={$action}" title="{$lang.helpbutton}" tabindex="4"><img src="{$dat}images/spacer.gif" width=100% height=100% alt="{$lang.helpbutton}" /></a></td></tr></a></table></td>
<table width=420>
...
<tr>
<td align="center" class="shlarge" rowspan=2 style="padding-top:6px; padding-bottom:14px;">
<form method="post" action="?pvtmarketbuy{$authstr}" name="pvmb">
...
<tr><td style="font-size:9px">{$types[pvm].name}</td>
    ...
    <td class="aright" style="font-size:9px"><input type="text" name="buy[{$types[pvm].type}]" size="4" value=""></td>
</tr>
{/section}

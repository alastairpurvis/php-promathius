...
*
*   $Id: registration.php,v 1.20.2.78 2006/12/17 10:51:27 acydburn Exp $
*
*
***************************************************************************/
...
a relevant message such as this in registration.php ... failure to do so 
...
function show_coppa()
{
    ...
    $template->set_filenames(array('body' => '../../../data/templates/agreement.tpl'));
    ...
    $template->assign_vars(array('REGISTRATION' => $lang['Registration'],
        ...
        'Agree_not'], "U_AGREE_OVER13" => append_sid("register.$phpEx?mode=register&amp;agreed=true"),
...
$error = false;
$error_msg = '';
$page_title = ($mode == 'editprofile') ? $lang['Edit_profile'] : $lang[
    'Register'];

if ($mode == 'register' && !isset($HTTP_POST_VARS['agreed']) && !isset($HTTP_GET_VARS
    ['agreed']))
{
    ...
    $mode == 'register')
{
    ...
    $template->assign_vars(array('STYLE_PBORDER_E' => 'post', 'IMG_CROSS_E' => '',
        'STYLE_PBORDER_U' => 'post', 'IMG_CROSS_U' => '', 'STYLE_PBORDER_P' => 'post',
        'IMG_CROSS_P' => '', 'STYLE_PBORDER_C' => 'post', 'IMG_CROSS_C' => ''));
...
//
if ($mode == 'register' && ($userdata['session_logged_in'] || $username == $userdata
    ['username']))
{
    message_die_ext(GENERAL_MESSAGE, $lang['Cannot_reg'], '', __line__, __file__);
}
...
<td class="row1" width="100%" style="text-align:center;padding-top:1px; padding-bottom:3px;padding-left:6px" colspan="2"><span style="font-size: 10px; color:#dd2d10">All fields must be filled in.</span>
</td>
</tr>', 'STYLE_PBORDER_E' => 'invalid-post', 'IMG_CROSS_E' =>
                    '<img src="data/images/gui/cross.gif" width=12 height=12></img>', 'STYLE_PBORDER_U' =>
                    'invalid-post', 'IMG_CROSS_U' =>
                    '<img src="data/images/gui/cross.gif" width=12 height=12></img>', 'STYLE_PBORDER_P' =>
                    'invalid-post', 'IMG_CROSS_P' =>
                    '<img src="data/images/gui/cross.gif" width=12 height=12></img>', 'STYLE_PBORDER_C' =>
                    'invalid-post', 'IMG_CROSS_C' =>
    ...
    if ($board_config['enable_confirm'] && $mode == 'register')
    {
        if (empty($HTTP_POST_VARS['confirm_id']))
        {
            ...
            $template->assign_vars(array('C_ERROR' =>
                '<br /><span class="gensmall" style="color:#dd2d10">' . $lang[
                'Confirm_code_wrong'] . '</span>', 'STYLE_PBORDER_C' => 'invalid-post',
                'IMG_CROSS_C' => '<img src="data/images/gui/cross.gif" width=12 height=12></img>'));
        }
        else
        {
            ...
            if (!($result = $db->sql_query($sql)))
            {
                message_die_ext(GENERAL_ERROR, 'Could not obtain confirmation code', '',
                    __line__, __file__, $sql);
            }
            ...
            if ($row = $db->sql_fetchrow($result))
            {
                ...
                else
                {
                    ...
                    if (!$db->sql_query($sql))
                    {
                        message_die_ext(GENERAL_ERROR, 'Could not delete confirmation code', '',
                            __line__, __file__, $sql);
                    }
                    else
                        $template->assign_vars(array('STYLE_PBORDER_C' => 'post', 'IMG_CROSS_C' => ''));
                }
            }
            else
            {
    ...
    if (!empty($new_password) && !empty($password_confirm))
    {
        ...
        {
            $error = true;
            $template->assign_vars(array('P_ERROR' =>
                '<br /><span class="gensmall" style="color:#dd2d10">' . $lang['Password_mismatch'] .
                '</span>', 'STYLE_PBORDER_P' => 'invalid-post', 'IMG_CROSS_P' =>
                '<img src="data/images/gui/cross.gif" width=12 height=12></img>'));
        }
        else
            if (strlen($new_password) > 32)
            {
                $error = true;
                $template->assign_vars(array('P_ERROR' =>
                    '<br /><span class="gensmall" style="color:#dd2d10">' . $lang['Password_long'] .
                    '</span>', 'STYLE_PBORDER_P' => 'invalid-post', 'IMG_CROSS_P' =>
                    '<img src="data/images/gui/cross.gif" width=12 height=12></img>'));
            }
            else
                if (strlen($new_password) < 4)
                {
                    $error = true;
                    $template->assign_vars(array('P_ERROR' =>
                        '<br /><span class="gensmall" style="color:#dd2d10">' . $lang['Password_short'] .
                        '</span>', 'STYLE_PBORDER_P' => 'invalid-post', 'IMG_CROSS_P' =>
                        '<img src="data/images/gui/cross.gif" width=12 height=12></img>'));
                }
                else
                {
                    $template->assign_vars(array('STYLE_PBORDER_P' => 'post', 'IMG_CROSS_P' => ''));
                    if (!$error)
                    {
    ...
    if ($email != $userdata['user_email'] || $mode == 'register')
    {
        ...
        if ($result['error'])
        {
            ...
            $template->assign_vars(array('E_ERROR' =>
                '<br /><span class="gensmall" style="color:#dd2d10">' . $result['error_msg'] .
                '</span>', 'STYLE_PBORDER_E' => 'invalid-post', 'IMG_CROSS_E' =>
                '<img src="data/images/gui/cross.gif" width=12 height=12></img>'));
        }
        else
        {
            ...
            if ($username != $userdata['username'] || $mode == 'register')
            {
                    ...
                    'register')
                {
                    ...
                    if ($result['error'])
                    {
                        ...
                        $template->assign_vars(array('U_ERROR' =>
                            '<br /><span class="gensmall" style="color:#dd2d10">' . $result['error_msg'] .
                            '</span>', 'STYLE_PBORDER_U' => 'invalid-post', 'IMG_CROSS_U' =>
                            '<img src="data/images/gui/cross.gif" width=12 height=12></img>'));
                    }
                    else
                    {
                        $template->assign_block_vars('switch_username_valid', array());
                        $template->assign_vars(array('STYLE_PBORDER_U' => 'post', 'IMG_CROSS_U' => ''));
                    }
                }
    ...
    if (!$error)
    {
        ...
        else
        {
            ...
            $sql = "INSERT INTO " . USERS_TABLE .
                ...
                str_replace("\'", "''", $user_dateformat) . "', '" . str_replace("\'", "''", $user_lang) .
                "', $user_style, 0, 1,";
                
                
    
                
            if ($board_config['require_once_activation'] == USER_ACTIVATION_SELF || $board_config
                ['require_once_activation'] == USER_ACTIVATION_ADMIN || $coppa)
            {
                $user_actkey = gen_rand_string(true);
                $key_len = 54 - (strlen($server_url));
                $key_len = ($key_len > 6) ? $key_len : 6;
                $user_actkey = substr($user_actkey, 0, $key_len);
                $sql .= "0, '" . str_replace("\'", "''", $user_actkey) . "','0', '1', '0', '0', '0', '', '', '', '', '')";
            }
            else
            {
                $sql .= "1, '','0', '1', '0', '0', '0', '', '', '', '', '')";
            ...
            {
                message_die_ext(GENERAL_ERROR, 'Could not insert data into users table', '',
                    __line__, __file__, $sql);
            }
            ...
            if (!($result = $db->sql_query($sql)))
            {
                message_die_ext(GENERAL_ERROR, 'Could not insert data into groups table', '',
                    __line__, __file__, $sql);
            }
            ...
            {
                message_die_ext(GENERAL_ERROR, 'Could not insert data into user_group table', '',
                    __line__, __file__, $sql);
            }
            ...
            $emailer->email_address($email);
            $emailer->set_subject(sprintf($lang['Welcome_subject'], $board_config[
                'sitename']));
            ...
            if ($coppa)
            {
                $emailer->assign_vars(array('SITENAME' => $board_config['sitename'],
                    ...
                    'EMAIL_SIG' => str_replace('<br />', "\n", "-- \n" . $board_config[
                    'board_email_sig']), 'FAX_INFO' => $board_config['coppa_fax'], 'MAIL_INFO' => $board_config
                    ['coppa_mail'], 'EMAIL_ADDRESS' => $email, 'ICQ' => $icq, 'AIM' => $aim, 'YIM' =>
                    $yim, 'MSN' => $msn, 'WEB_SITE' => $website, 'FROM' => $location, 'OCC' => $occupation,
            ...
            $message = $message . '<br /><br /><br />' . sprintf($lang['Click_return_login1'],
                '<a href="' . append_sid("login.$phpEx") . '">', '</a>');

            message_die_ext(ACCOUNT_CREATED, $message);
        }// if mode == register
    }
...
else
{
    ...
    $template->set_filenames(array('body' =>
        '../../../data/templates/register_form.tpl'));
    ...
    if (!empty($board_config['enable_confirm']) && $mode == 'register')
    {
        ...
        $sql = 'SELECT COUNT(session_id) AS ts 
			FROM ' . CONFIRM_TABLE . " 
        ...
        if ($row = $db->sql_fetchrow($result))
        {
            if ($row['attempts'] > 5)
            {
                message_die_ext(GENERAL_MESSAGE, $lang['Too_many_registers']);
            }
        }
        $db->sql_freeresult($result);
        ...
        $confirm_image = '<img src="' . append_sid("register.$phpEx?mode=confirm&amp;id=$confirm_id") .
            '" alt="" title="" />';
    ...
    $template->assign_vars(array('USERNAME' => isset($username) ? $username : '',
        ...
        'register') ? $lang['Password'] : $lang['New_password'], 'L_CONFIRM_PASSWORD' =>
        ...
        $form_enctype, 'S_PROFILE_ACTION' => append_sid("register.$phpEx")));

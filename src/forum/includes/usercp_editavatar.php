...
$error = FALSE;
$error_msg = '';
$page_title = ( $mode == 'editavatar' ) ? $lang['Edit_profile'] : $lang['Register'];
	...
	$mode == 'register' )
{
	...
	$user_avatar = ( empty($user_avatar_local) && $mode == 'editavatar' ) ? $userdata['user_avatar'] : '';
	$user_avatar_type = ( empty($user_avatar_local) && $mode == 'editavatar' ) ? $userdata['user_avatar_type'] : '';
...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	$avatar_sql = "user_avatar = '" . $userdata[user_avatar] . "', user_avatar_type = " . $userdata[user_avatar_type];
	if ( isset($HTTP_POST_VARS['avatardel']) && $mode == 'editavatar' )
	{
	...
	if ( !$error )
	{
		if ( $avatar_sql == '' )
		{
			$avatar_sql = ( $mode == 'editavatar' ) ? '' : "'', " . USER_AVATAR_NONE;
		}
		...
		if ( $mode == 'editavatar' )
		{
			...
			if ( !($result = $db->sql_query($sql)) )
			{
				message_die_ext(GENERAL_ERROR, 'Could not update users table' . $avatar_sql, '', __LINE__, __FILE__, $sql);
			}
			...
			{
				$message = $lang['Profile_updated'] . '<br /><br />' . sprintf($lang['Click_return_cp'],  '<a href="' . append_sid("ucp.$phpEx?mode=editavatar") . '">', '</a>');
			}

			$template->assign_vars(array(
				"META" => '<meta http-equiv="refresh" content="5;url=' . append_sid("ucp.$phpEx?mode=editavatar") . '">')
			);
...
else if ( $mode == 'editavatar' && !isset($HTTP_POST_VARS['avatargallery']) && !isset($HTTP_POST_VARS['submitavatar']) && !isset($HTTP_POST_VARS['cancelavatar']) )
{
...
else
{
	...
	if ( $user_avatar_type )
	{
		switch( $user_avatar_type )
		{
			case USER_AVATAR_UPLOAD:
				$avatar_img = ( $board_config['allow_avatar_upload'] ) ? '<img src="' . $phpbb_root_path . $board_config['avatar_path'] . '/' . $user_avatar . '" alt="" />' : '';
				break;
			case USER_AVATAR_REMOTE:
				$avatar_img = ( $board_config['allow_avatar_remote'] ) ? '<img src="' . $phpbb_root_path . $user_avatar . '" alt="" />' : '';
				break;
			case USER_AVATAR_GALLERY:
				$avatar_img = ( $board_config['allow_avatar_local'] ) ? '<img src="' . $phpbb_root_path . $board_config['avatar_gallery_path'] . '/' . $user_avatar . '" alt="" />' : '';
				break;
		}
	}
	...
	$template->set_filenames(array(
		'body' => '../../../data/templates/ucp/profile_avatar.tpl')
	);
...
{
	$template->assign_vars(array('URL' => $userdata[user_avatar]));
}
	...
	$template->assign_vars(array(
		...
		'L_CONFIRM_PASSWORD_EXPLAIN' => ( $mode == 'editavatar' ) ? $lang['Confirm_password_explain'] : '',
		'L_PASSWORD_IF_CHANGED' => ( $mode == 'editavatar' ) ? $lang['password_if_changed'] : '',
		'L_PASSWORD_CONFIRM_IF_CHANGED' => ( $mode == 'editavatar' ) ? $lang['password_confirm_if_changed'] : '',
		...
		'L_AVATAR_EXPLAIN' => sprintf($lang['Avatar_explain'], (round($board_config['avatar_filesize'] / 1024))),

...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	if ( $result = $db->sql_query($sql) )
	{
		if ( $row = $db->sql_fetchrow($result) )
		{
			if ( !$row['user_active'] )
			{
				message_die_ext(GENERAL_MESSAGE, $lang['No_send_account_inactive']);
			}
			...
			if ( !$db->sql_query($sql) )
			{
				message_die_ext(GENERAL_ERROR, 'Could not update new password information', '', __LINE__, __FILE__, $sql);
			}
		...
		{
			message_die_ext(GENERAL_MESSAGE, $lang['No_email_match']);
		}
	}
	else
	{
		message_die_ext(GENERAL_ERROR, 'Could not obtain user information for sendpassword', '', __LINE__, __FILE__, $sql);
	}
}
else
{
...
$template->assign_vars(array(
	...
	'S_HIDDEN_FIELDS' => '', 
	'S_PROFILE_ACTION' => append_sid("sendpass.$phpEx?mode=sendpassword"))
);

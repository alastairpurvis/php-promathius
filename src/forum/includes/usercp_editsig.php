 ...
 *
 *   $Id: usercp_edisig.php,v 1.20.2.78 2006/12/17 10:51:27 acydburn Exp $
 *
 *
 ***************************************************************************/
...
/*
	...
	a relevant message such as this in usercp_edisig.php ... failure to do so 
...
function show_coppa()
{
	...
	$template->assign_vars(array(
		...
		"U_AGREE_OVER13" => append_sid("ucp.$phpEx?mode=edisig&amp;agreed=true"),
		"U_AGREE_UNDER13" => append_sid("ucp.$phpEx?mode=edisig&amp;agreed=true&amp;coppa=true"))
	);
...
$error = FALSE;
$error_msg = '';
$page_title = ( $mode == 'editsig' ) ? $lang['Edit_profile'] : $lang['edisig'];
	...
	$mode == 'register' )
{
	...
	$user_avatar = ( empty($user_avatar_local) && $mode == 'editsig' ) ? $userdata['user_avatar'] : '';
	$user_avatar_type = ( empty($user_avatar_local) && $mode == 'editsig' ) ? $userdata['user_avatar_type'] : '';
...
//
// Let's make sure the user isn't logged in while 'register'ing,
...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	if ( isset($HTTP_POST_VARS['avatardel']) && $mode == 'editsig' )
	{
	...
	if ( !$error )
	{
		if ( $avatar_sql == '' )
		{
			$avatar_sql = ( $mode == 'editsig' ) ? '' : "'', " . USER_AVATAR_NONE;
		}
		...
		if ( $mode == 'editsig' )
		{
			...
			$sql = "UPDATE " . USERS_TABLE . "
				SET user_sig = '" . str_replace("\'", "''", $signature) . "', user_sig_bbcode_uid = '$signature_bbcode_uid'
			...
			{
				$message = $lang['Profile_updated'] . '<br /><br />' . sprintf($lang['Click_return_cp'],  '<a href="' . append_sid("ucp.$phpEx?mode=editsig") . '">', '</a>');
			}

			$template->assign_vars(array(
				"META" => '<meta http-equiv="refresh" content="5;url=' . append_sid("ucp.$phpEx?mode=editsig") . '">')
			);
...
else if ( $mode == 'editsig' && !isset($HTTP_POST_VARS['avatargallery']) && !isset($HTTP_POST_VARS['submitavatar']) && !isset($HTTP_POST_VARS['cancelavatar']) )
{
...
else
{
	...
	$template->set_filenames(array(
		'body' => '../../../data/templates/ucp/profile_editsig.tpl')
	);
	...
	$template->assign_vars(array(
		...
		'BBCODE_STATUS' => sprintf($bbcode_status, '<a href="' . append_sid("forum/faq.$phpEx?mode=bbcode") . '" target="_phpbbcode">', '</a>'),
		'SMILIES_STATUS' => $smilies_status,

		'L_CURRENT_PASSWORD' => $lang['Current_password'],
		'L_NEW_PASSWORD' => ( $mode == 'edisig' ) ? $lang['Password'] : $lang['New_password'],
		'L_CONFIRM_PASSWORD' => $lang['Confirm_password'],
		'L_CONFIRM_PASSWORD_EXPLAIN' => ( $mode == 'editsig' ) ? $lang['Confirm_password_explain'] : '',
		'L_PASSWORD_IF_CHANGED' => ( $mode == 'editsig' ) ? $lang['password_if_changed'] : '',
		'L_PASSWORD_CONFIRM_IF_CHANGED' => ( $mode == 'editsig' ) ? $lang['password_confirm_if_changed'] : '',

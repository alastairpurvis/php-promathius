...
function submit_post($mode, &$post_data, &$message, &$meta, &$forum_id, &$topic_id, &$post_id, &$poll_id, &$topic_type, &$bbcode_on, &$html_on, &$smilies_on, &$attach_sig, &$bbcode_uid, $post_username, $post_subject, $post_message, $poll_title, &$poll_options, &$poll_length)
{
	...
	if ($mode == 'newtopic' || ($mode == 'editpost' && $post_data['first_post']))
	{
		...
		if (!$db->sql_query($sql))
		{
			message_die_ext(GENERAL_ERROR, 'Error in posting', '', __LINE__, __FILE__, $sql);
		}
	...
	if (($mode == 'newtopic' || ($mode == 'editpost' && $post_data['edit_poll'])) && !empty($poll_title) && count($poll_options) >= 2)
	{
		...
		if ($mode == 'editpost' && $post_data['has_poll'])
		{
			...
			if (!($result = $db->sql_query($sql)))
			{
				message_die_ext(GENERAL_ERROR, 'Could not obtain vote data results for this topic', '', __LINE__, __FILE__, $sql);
			}
		...
		if ($delete_option_sql != '')
		{
			...
			if (!$db->sql_query($sql))
			{
				message_die_ext(GENERAL_ERROR, 'Error deleting pruned poll options', '', __LINE__, __FILE__, $sql);
			}
		}
	}
	...
	if ($mode == 'delete')
	{
		if ($post_data['last_post'])
		{
			...
			else
			{
				...
				if (!($result = $db->sql_query($sql)))
				{
					message_die_ext(GENERAL_ERROR, 'Error in deleting post', '', __LINE__, __FILE__, $sql);
				}
...
function delete_post($mode, &$post_data, &$message, &$meta, &$forum_id, &$topic_id, &$post_id, &$poll_id)
{
	...
	if ($mode == 'poll_delete' || ($mode == 'delete' && $post_data['first_post'] && $post_data['last_post']) && $post_data['has_poll'] && $post_data['edit_poll'])
	{
		...
		if (!$db->sql_query($sql))
		{
			message_die_ext(GENERAL_ERROR, 'Error in deleting poll', '', __LINE__, __FILE__, $sql);
		}
...
function user_notification($mode, &$post_data, &$topic_title, &$forum_id, &$topic_id, &$post_id, &$notify_user)
{
	...
	if ($mode != 'delete')
	{
		if ($mode == 'reply')
		{
			...
			if (!($result = $db->sql_query($sql)))
			{
				message_die_ext(GENERAL_ERROR, 'Could not obtain banlist', '', __LINE__, __FILE__, $sql);
			}
			...
			if (!($result = $db->sql_query($sql)))
			{
				message_die_ext(GENERAL_ERROR, 'Could not obtain list of topic watchers', '', __LINE__, __FILE__, $sql);
			}
		...
		if (!($result = $db->sql_query($sql)))
		{
			message_die_ext(GENERAL_ERROR, 'Could not obtain topic watch information', '', __LINE__, __FILE__, $sql);
		}
		...
		if (!$notify_user && !empty($row['topic_id']))
		{
			...
			if (!$db->sql_query($sql))
			{
				message_die_ext(GENERAL_ERROR, 'Could not delete topic watch information', '', __LINE__, __FILE__, $sql);
			}
		}
		else if ($notify_user && empty($row['topic_id']))
		{
			...
			if (!$db->sql_query($sql))
			{
				message_die_ext(GENERAL_ERROR, 'Could not insert topic watch information', '', __LINE__, __FILE__, $sql);
			}
		}
	}
}

...
require_once($game_root_path."/lib/magicfun.php");

global $authstr;

if ($config['magic']['hidden'] == True)
	$begin = 300;
else
	$begin = 201;
$where = "(code > $begin AND code < 400) ";
...
while ($new = mysql_fetch_array($news)) {
	...
	$id2 = $new[id2];
	$id3 = $new[id3];
	$name1 = '<a class=proflink href=?profiles&num='.$id1.$authstr.'>'.sqlsafeeval("SELECT empire FROM $playerdb WHERE num=$id1;").($new[online]?'*':'').'</a>';
	$name2 = '<a class=proflink href=?profiles&num='.$id2.$authstr.'>'.sqlsafeeval("SELECT empire FROM $playerdb WHERE num=$id2;").'</a>';
	$name3 = '<a class=proflink href=?profiles&num='.$id3.$authstr.'>'.sqlsafeeval("SELECT empire FROM $playerdb WHERE num=$id3;").'</a>';
	$clan1 = $new[clan1];
	$clan2 = $new[clan2];
	$clan3 = $new[clan3];
	...
	if($clan2 != 0)		$clname2 = sqlsafeeval("SELECT name FROM $clandb WHERE num=$clan2;").' <a class=proflink href=?clancrier&sclan='.
					$clan2.$authstr.'>('.sqlsafeeval("SELECT tag FROM $clandb WHERE num=$clan2;").')</a>';
	else			$clname2 = "None";
	if($clan3 != 0)		$clname3 = sqlsafeeval("SELECT name FROM $clandb WHERE num=$clan3;").' <a class=proflink href=?clancrier&sclan='.
					$clan3.$authstr.'>('.sqlsafeeval("SELECT tag FROM $clandb WHERE num=$clan3;").')</a>';
	else			$clname3 = "None";
	...
	if(floor($code/100) == 3) {
		...
		if($type == 99) {
			...
			switch($shielded) {
				case 1:
					$what_for = " (land)";
					if($new[land1] == 0)	$outcome = "Defence Held";
					else			$outcome = commas($new[land1])." acres";
					break;
				case 2:
					$what_for = " (cash)";
					if($new[cash1] == 0)	$outcome = "Defence Held";
					else			$outcome = '$'.commas($new[cash1]);
					break;
				case 3:
					$what_for = " (food)";
					if($new[food1] == 0)	$outcome = "Defence Held";
		...
		if($type == 11) {
			switch($shielded) {
				...
				case  1:	$outcome = "Defence Held";			break;
			}
		} else {
			switch($shielded) {
				case -1:	$outcome = "Foiled";		break;
...
$liveclans = array();
$deadclans = array();
$list = mysql_safe_query("SELECT num,name,tag FROM $clandb WHERE members>0 ORDER BY num;");
while($clan = mysql_fetch_array($list))
	$liveclans[] = $clan;

$list = mysql_safe_query("SELECT num,name,tag FROM $clandb WHERE members=-1 ORDER BY num;");
while($clan = mysql_fetch_array($list))
	$deadclans[] = $clan;

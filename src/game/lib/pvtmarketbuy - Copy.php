...
function getBuyCosts ($type, $num='') {
	...
	$costbonus = 1 - ((1-$config[mktshops])*(($users[barracks]*$config['buildingoutput']) / $users[land]) + $config[mktshops]*(($users[shops]*$config['buildingoutput']) / $users[land]));
	if ($type != 'troop')
...
function buyUnits ($type, $num='') {
	...
	if ($amount == 0)
		return;
	elseif ($amount < 0)
		$msg_error1 .= "Cannot purchase a negative amount of $uera[$ts].";
	elseif ($amount > $umktmt)
		$msg_error1 .= "Not enough $uera[$ts] available.";
	elseif ($cost > $users[cash])
		$msg_error1 .= "Not enough money to buy ".commas(gamefactor($amount))." $uera[$ts].";
	else
	{
		$users[cash] -= $cost;
		if($type == 'troop') {
		...
		$msg[] = commas(gamefactor($amount))." $itemname purchased for ".commas(gamefactor($cost))." gold";
	}
}

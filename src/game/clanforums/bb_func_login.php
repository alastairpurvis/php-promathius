...
if(($action=='pthread' and isset($_POST['postText']) and $_POST['postText']=='') OR ($action=='ptopic' and isset($_POST['topicText']) and $_POST['topicText']=='')) $action='';
...
if ($action=='') {
if(isset($metaLocation)) { $meta_relocate="{$main_url}/{$indexphp}{$queryStr}"; echo ParseTpl(makeUp($metaLocation)); exit; } else header("Location: {$main_url}/{$indexphp}{$queryStr}");
}
}
else {
...
require_once($game_root_path.'/clanforums/bb_func_txt.php');$reqTxt=1;

...
if ($user_id!=0 and $row=db_simpleSelect(0,"$Tp,$Tt","$Tp.post_text, $Tt.topic_title, $Tp.post_time","$Tp.post_id",'=',$post,'','',"$Tp.topic_id",'=',"$Tt.topic_id") and (!isset($useredit) or $useredit==0 or $useredit>(strtotime(date('Y-m-d H:i:s'))-strtotime($row[2])) or $user_id==1 or $isMod==1) and ($userAllow=db_simpleSelect(0,$Tp,'poster_id,post_status','post_id','=',$post,'','','poster_id','=',$user_id) or $user_id==1 or $isMod==1) ) {
...
$title.=$l_editPost;
$correctErr="<a href=\"{$indexphp}action=vthread&amp;forum=$forum&amp;topic=$topic&amp;page={$page}#{$anchor}\">$l_back</a>";
}

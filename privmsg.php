...
include($phpbb_root_path . 'includes/bbcode_ext.'.$phpEx);
include($phpbb_root_path . 'includes/functions_post_ext.'.$phpEx);
...
{
	message_die_ext(GENERAL_MESSAGE, 'PM_disabled');
}
...
//
$inbox_img = ( $folder != 'inbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=inbox") . '"><img src="' . $images['pm_inbox'] . '" alt="' . $lang['Inbox'] . '" /></a>' : '<img src="' . $images['pm_inbox'] . '" alt="' . $lang['Inbox'] . '" />';
$inbox_url = ( $folder != 'inbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=inbox") . '">' . $lang['Inbox'] . '</a>' : $lang['Inbox'];

$outbox_img = ( $folder != 'outbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=outbox") . '"><img src="' . $images['pm_outbox'] . '" alt="' . $lang['Outbox'] . '" /></a>' : '<img src="' . $images['pm_outbox'] . '" alt="' . $lang['Outbox'] . '" />';
$outbox_url = ( $folder != 'outbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=outbox") . '">' . $lang['Outbox'] . '</a>' : $lang['Outbox'];

$sentbox_img = ( $folder != 'sentbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=sentbox") . '"><img src="' . $images['pm_sentbox'] . '" alt="' . $lang['Sentbox'] . '" /></a>' : '<img src="' . $images['pm_sentbox'] . '" alt="' . $lang['Sentbox'] . '" />';
$sentbox_url = ( $folder != 'sentbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=sentbox") . '">' . $lang['Sentbox'] . '</a>' : $lang['Sentbox'];

$savebox_img = ( $folder != 'savebox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=savebox") . '"><img src="' . $images['pm_savebox'] . '" alt="' . $lang['Savebox'] . '" /></a>' : '<img src="' . $images['pm_savebox'] . '" alt="' . $lang['Savebox'] . '" />';
...
else if ( $mode == 'read' )
{
	...
	{
		message_die_ext(GENERAL_ERROR, $lang['No_post_id']);
	}
	...
	switch( $folder )
	{
		...
		case 'savebox':
			...
			break;
		default:
			message_die_ext(GENERAL_ERROR, $lang['No_such_folder']);
			break;
	}
	...
	//
	$sql = "SELECT u.username AS username_1, u.user_id AS user_id_1, u2.username AS username_2, u2.user_id AS user_id_2, u.user_sig_bbcode_uid, u.user_posts, u.user_from, u.user_website, u.user_email, u.user_icq, u.user_aim, u.user_yim, u.user_regdate, u.user_msnm, u.user_viewemail, u.user_rank, u.user_sig, u.user_avatar, u.user_avatar_type, u.user_allowavatar, u.is_empire, u.user_showempire, u.empire, pm.*, pmt.privmsgs_bbcode_uid, pmt.privmsgs_text
	...
	{
		message_die_ext(GENERAL_ERROR, 'Could not query private message post information', '', __LINE__, __FILE__, $sql);
	}
	...
	if (($privmsg['privmsgs_type'] == PRIVMSGS_NEW_MAIL || $privmsg['privmsgs_type'] == PRIVMSGS_UNREAD_MAIL) && $folder == 'inbox')
	{
		...
		if ( !$db->sql_query($sql) )
		{
			message_die_ext(GENERAL_ERROR, 'Could not update private message read status for user', '', __LINE__, __FILE__, $sql);
		}
		...
		if ( !$db->sql_query($sql) )
		{
			message_die_ext(GENERAL_ERROR, 'Could not update private message read status', '', __LINE__, __FILE__, $sql);
		}
		...
		if ( !($result = $db->sql_query($sql)) )
		{
			message_die_ext(GENERAL_ERROR, 'Could not obtain sent message info for sendee', '', __LINE__, __FILE__, $sql);
		}
		...
		if ( $sent_info = $db->sql_fetchrow($result) )
		{
			if ($board_config['max_sentbox_privmsgs'] && $sent_info['sent_items'] >= $board_config['max_sentbox_privmsgs'])
			{
				...
				if ( !$result = $db->sql_query($sql) )
				{
					message_die_ext(GENERAL_ERROR, 'Could not find oldest privmsgs', '', __LINE__, __FILE__, $sql);
				}
				...
				if ( !$db->sql_query($sql) )
				{
					message_die_ext(GENERAL_ERROR, 'Could not delete oldest privmsgs (sent)', '', __LINE__, __FILE__, $sql);
				}
				...
				if ( !$db->sql_query($sql) )
				{
					message_die_ext(GENERAL_ERROR, 'Could not delete oldest privmsgs text (sent)', '', __LINE__, __FILE__, $sql);
				}
			}
		}
		...
		if ( !$db->sql_query($sql) )
		{
			message_die_ext(GENERAL_ERROR, 'Could not insert private message sent info', '', __LINE__, __FILE__, $sql);
		}
		...
		if ( !$db->sql_query($sql) )
		{
			message_die_ext(GENERAL_ERROR, 'Could not insert private message sent text', '', __LINE__, __FILE__, $sql);
		}
	}
	...
	$post_date = create_date($board_config['default_dateformat'], $privmsg['privmsgs_date'], $board_config['board_timezone'], true);

	$temp_url = append_sid("forum/profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '=' . $user_id_from);
	...
	{
		$email_uri = ( $board_config['board_email_form'] ) ? append_sid("forum/profile.$phpEx?mode=email&amp;" . POST_USERS_URL .'=' . $user_id_from) : 'mailto:' . $privmsg['user_email'];
	...
	$temp_url = append_sid($phpbb_root_path . "search.$phpEx?search_author=" . urlencode($username_from) . "&amp;showresults=posts");
	...
	if ( $privmsg['user_avatar_type'] && $privmsg['user_allowavatar'] )
	{
		switch($privmsg['user_avatar_type'])
		{
			case USER_AVATAR_UPLOAD:
				$poster_avatar = ( $board_config['allow_avatar_upload'] ) ? '<img src="forum/' . $board_config['avatar_path'] . '/' . $privmsg['user_avatar'] . '" alt="" />' : '';
				break;
			case USER_AVATAR_REMOTE:
				$poster_avatar = ( $board_config['allow_avatar_remote'] ) ? '<img src="forum/' . $privmsg['user_avatar'] . '" alt="" />' : '';
				break;
			case USER_AVATAR_GALLERY:
				$poster_avatar = ( $board_config['allow_avatar_local'] ) ? '<img src="forum/' . $board_config['avatar_gallery_path'] . '/' . $privmsg['user_avatar'] . '" alt="" />' : '';
				break;
		}
	}
	$poster_posts = ( $privmsg['user_id'] != ANONYMOUS ) ? $lang['Posts'] . ': ' . $privmsg['user_posts'] : '';
	$poster_empire = ( $privmsg['is_empire'] != '0' && $privmsg['user_showempire'] != 0) ? "<br />
				<b>Empire</b><br />" . $privmsg[empire]."</i>" : '';

	$poster_from = ( $privmsg['user_from'] && $privmsg['user_id'] != ANONYMOUS ) ? $lang['Location'] . ': ' . $privmsg['user_from'] : '';

	$poster_joined = ( $privmsg['user_id'] != ANONYMOUS ) ? $lang['Joined'] . ': ' . create_date($lang['DATE_FORMAT'], $privmsg['user_regdate'], $board_config['board_timezone']) : '';
...
else if ( ( $delete && $mark_list ) || $delete_all )
{
	...
	else if ( $confirm )
	{
		...
		if ( !($result = $db->sql_query($sql)) )
		{
			message_die_ext(GENERAL_ERROR, 'Could not obtain id list to delete messages', '', __LINE__, __FILE__, $sql);
		}
		...
		if ( count($mark_list) )
		{
			...
			if ($folder == 'inbox' || $folder == 'outbox')
			{
				...
				if ( !($result = $db->sql_query($sql)) )
				{
					message_die_ext(GENERAL_ERROR, 'Could not obtain user id list for outbox messages', '', __LINE__, __FILE__, $sql);
				}
					...
					if (sizeof($update_users))
					{
						...
						while (list($type, $dec_ary) = each($update_list))
						{
							...
							while (list($dec, $user_ary) = each($dec_ary))
							{
								...
								if ( !$db->sql_query($sql) )
								{
									message_die_ext(GENERAL_ERROR, 'Could not update user pm counters', '', __LINE__, __FILE__, $sql);
								}
							}
						}
						unset($update_list);
			...
			{
				message_die_ext(GENERAL_ERROR, 'Could not delete private message info', '', __LINE__, __FILE__, $delete_sql);
			}

			if ( !$db->sql_query($delete_text_sql, END_TRANSACTION) )
			{
				message_die_ext(GENERAL_ERROR, 'Could not delete private message text', '', __LINE__, __FILE__, $delete_text_sql);
			}
		}
	}
}
	...
	if (sizeof($mark_list))
	{
		...
		if ( $saved_info = $db->sql_fetchrow($result) )
		{
			if ($board_config['max_savebox_privmsgs'] && $saved_info['savebox_items'] >= $board_config['max_savebox_privmsgs'] )
			{
				...
				if ( !$result = $db->sql_query($sql) )
				{
					message_die_ext(GENERAL_ERROR, 'Could not find oldest privmsgs (save)', '', __LINE__, __FILE__, $sql);
				}
				$old_privmsgs_id = $db->sql_fetchrow($result);
				$old_privmsgs_id = $old_privmsgs_id['privmsgs_id'];
				...
				if ( !$db->sql_query($sql) )
				{
					message_die_ext(GENERAL_ERROR, 'Could not delete oldest privmsgs (save)', '', __LINE__, __FILE__, $sql);
				}
				...
				if ( !$db->sql_query($sql) )
				{
					message_die_ext(GENERAL_ERROR, 'Could not delete oldest privmsgs text (save)', '', __LINE__, __FILE__, $sql);
				}
			}
		}
		...
		{
			message_die_ext(GENERAL_ERROR, 'Could not save private messages', '', __LINE__, __FILE__, $saved_sql);
		}
...
else if ( $submit || $refresh || $mode != '' )
{
	...
	if ( $submit && $mode != 'edit' )
	{
		...
		if ( $result = $db->sql_query($sql) )
		{
			...
			{
				message_die_ext(GENERAL_MESSAGE, $lang['Flood_Error']);
			}
		}
		//
		// End Flood control
	...
	if ($submit && $mode == 'edit')
	{
		...
		if (!($result = $db->sql_query($sql)))
		{
			message_die_ext(GENERAL_ERROR, "Could not obtain message details", "", __LINE__, __FILE__, $sql);
		}

		if (!($row = $db->sql_fetchrow($result)))
		{
			message_die_ext(GENERAL_MESSAGE, $lang['No_such_post']);
		}
		$db->sql_freeresult($result);
	...
	if ( $submit && !$error )
	{
		...
		if ( $mode != 'edit' )
		{
			...
			if ( !($result = $db->sql_query($sql)) )
			{
				message_die_ext(GENERAL_MESSAGE, $lang['No_such_user']);
			}
			...
			if ( $inbox_info = $db->sql_fetchrow($result) )
			{
				if ($board_config['max_inbox_privmsgs'] && $inbox_info['inbox_items'] >= $board_config['max_inbox_privmsgs'])
				{
					...
					if ( !$result = $db->sql_query($sql) )
					{
						message_die_ext(GENERAL_ERROR, 'Could not find oldest privmsgs (inbox)', '', __LINE__, __FILE__, $sql);
					}
					$old_privmsgs_id = $db->sql_fetchrow($result);
					$old_privmsgs_id = $old_privmsgs_id['privmsgs_id'];
					...
					if ( !$db->sql_query($sql) )
					{
						message_die_ext(GENERAL_ERROR, 'Could not delete oldest privmsgs (inbox)'.$sql, '', __LINE__, __FILE__, $sql);
					}
					...
					if ( !$db->sql_query($sql) )
					{
						message_die_ext(GENERAL_ERROR, 'Could not delete oldest privmsgs text (inbox)', '', __LINE__, __FILE__, $sql);
					}
				}
			}
		...
		{
			message_die_ext(GENERAL_ERROR, "Could not insert/update private message sent info.", "", __LINE__, __FILE__, $sql_info);
		}
		...
		{
			message_die_ext(GENERAL_ERROR, "Could not insert/update private message sent text.", "", __LINE__, __FILE__, $sql);
		}
		...
		if ( $mode != 'edit' )
		{
			...
			{
				message_die_ext(GENERAL_ERROR, 'Could not update private message new/read status for user', '', __LINE__, __FILE__, $sql);
			}
		...
		message_die_ext(GENERAL_MESSAGE, $msg);
	}
	else if ( $preview || $refresh || $error )
	{
		...
		else if ( $mode == 'edit' )
		{
			...
			if ( !($result = $db->sql_query($sql)) )
			{
				message_die_ext(GENERAL_ERROR, "Could not obtain post and post text", "", __LINE__, __FILE__, $sql);
			}
			...
			if ( $postrow = $db->sql_fetchrow($result) )
			{
				...
				{
					message_die_ext(GENERAL_MESSAGE, $lang['Edit_own_posts']);
				}
	...
	else 
	{
		...
		else if ( $mode == 'edit' )
		{
			...
			if ( !($result = $db->sql_query($sql)) )
			{
				message_die_ext(GENERAL_ERROR, 'Could not obtain private message for editing', '', __LINE__, __FILE__, $sql);
			}
		...
		else if ( $mode == 'reply' || $mode == 'quote' )
		{
			...
			if ( $mode == 'quote' )
			{
				...
				$msg_date =  create_date($board_config['default_dateformat'], $privmsg['privmsgs_date'], $board_config['board_timezone'], true); 
	...
	$poster_avatar = '';
	if ( $userdata['user_avatar_type'] && $poster_id != ANONYMOUS && $userdata['user_allowavatar'] )
	{
		switch( $userdata['user_avatar_type'] )
		{
			case USER_AVATAR_UPLOAD:
				$poster_avatar = ( $board_config['allow_avatar_upload'] ) ? '<img src="forum/' . $board_config['avatar_path'] . '/' . $userdata['user_avatar'] . '" alt="" />' : '';
				break;
			case USER_AVATAR_REMOTE:
				$poster_avatar = ( $board_config['allow_avatar_remote'] ) ? '<img src="forum/' . $userdata['user_avatar'] . '" alt="" />' : '';
				break;
			case USER_AVATAR_GALLERY:
				$poster_avatar = ( $board_config['allow_avatar_local'] ) ? '<img src="forum/' . $board_config['avatar_gallery_path'] . '/' . $userdata['user_avatar'] . '" alt="" />' : '';
				break;
		}
	}
...
if ( !$db->sql_query($sql) )
{
	message_die_ext(GENERAL_ERROR, 'Could not update private message new/read status (2) for user', '', __LINE__, __FILE__, $sql);
}
...
switch( $folder )
{
	...
	default:
		message_die_ext(GENERAL_MESSAGE, $lang['No_such_folder']);
		break;
}
...
$sql .= $limit_msg_time . " ORDER BY pm.privmsgs_date DESC LIMIT $start, " . $board_config['pms_per_page'];
$sql_all_tot = $sql_tot;
...
{
	message_die_ext(GENERAL_ERROR, 'Could not query private message information', '', __LINE__, __FILE__, $sql_tot);
}
...
if ( !($result = $db->sql_query($sql)) )
{
	message_die_ext_ext(GENERAL_ERROR, 'Could not query private messages', '', __LINE__, __FILE__, $sql);
}
...
if ( $row = $db->sql_fetchrow($result) )
{
	...
	do
	{
		...
		$u_from_user_profile = append_sid("forum/profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$msg_userid");
		...
		$msg_date = create_date($board_config['default_dateformat'], $row['privmsgs_date'], $board_config['board_timezone'],true);

		if ( $flag == PRIVMSGS_NEW_MAIL  || $flag == PRIVMSGS_UNREAD_MAIL )
		{
	...
	$template->assign_vars(array(
		'PAGINATION' => generate_pagination("privmsg.$phpEx?folder=$folder", $pm_total, $board_config['pms_per_page'], $start),
		'PAGE_NUMBER' => sprintf($lang['Page_of'], ( floor( $start / $board_config['pms_per_page'] ) + 1 ), ceil( $pm_total / $board_config['pms_per_page'] )), 
...
$template->assign_vars(array('PATH' => $phpbb_root_path));
$template->pparse('body');

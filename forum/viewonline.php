...
while ( $row = $db->sql_fetchrow($result) )
{
	...
	if ( $view_online )
	{
		if ( $row['session_page'] < 1 || !$is_auth_ary[$row['session_page']]['auth_view'] )
		{
			switch( $row['session_page'] )
			{
				...
				case PAGE_FAQ:
					...
					break;
				case PAGE_PROMATHIUS:
					$location = $lang['Playing_Promathius'];
					$location_url = "../?main.";
					break;
				case PAGE_EMPIRE:
					$location = 'Establishing an empire';
					$location_url = "../?main.";
					break;
				default:
					$location = "Unknown";
		...
		$template->assign_block_vars("$which_row", array(
			...
			'ROW_CLASS' => $row_class,
			'USERNAME' => $username,
			'LASTUPDATE' => create_date($board_config['default_dateformat'], $row['session_time'], $board_config['board_timezone'], true),

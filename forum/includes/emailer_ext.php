...
class emailer
{
	...
	function use_template($template_file, $template_lang = '')
	{
		...
		{
			message_die_ext_ext(GENERAL_ERROR, 'No template file set', '', __LINE__, __FILE__);
		}
		...
		if (empty($this->tpl_msg[$template_lang . $template_file]))
		{
			...
			if (!@file_exists(@phpbb_realpath($tpl_file)))
			{
				...
				if (!@file_exists(@phpbb_realpath($tpl_file)))
				{
					message_die_ext(GENERAL_ERROR, 'Could not find email template file :: ' . $template_file, '', __LINE__, __FILE__);
				}
			}

			if (!($fd = @fopen($tpl_file, 'r')))
			{
				message_die_ext(GENERAL_ERROR, 'Failed opening template file :: ' . $tpl_file, '', __LINE__, __FILE__);
			}
	...
	function send()
	{
		...
		else
		{
			...
			if (!$result && !$board_config['sendmail_fix'] && $empty_to_header)
			{
				...
				if (!$db->sql_query($sql))
				{
					message_die_ext(GENERAL_ERROR, 'Unable to update config table', '', __LINE__, __FILE__, $sql);
				}
		...
		if (!$result)
		{
			message_die_ext(GENERAL_ERROR, 'Failed sending email (' . (($this->use_smtp) ? 'SMTP' : 'PHP') . ') ' . $result, '', __LINE__, __FILE__);
		}

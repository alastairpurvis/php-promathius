...
function user_avatar_delete($avatar_type, $avatar_file)
{
	...
	{
		if ( @file_exists(@phpbb_realpath('./forum/' . $board_config['avatar_path'] . '/' . $avatar_file)) )
		{
			@unlink('./forum/' . $board_config['avatar_path'] . '/' . $avatar_file);
		}
	}
...
function user_avatar_gallery($mode, &$error, &$error_msg, $avatar_filename, $avatar_category)
{
	...
	if ( file_exists(@phpbb_realpath($board_config['avatar_gallery_path'] . '/' . $avatar_category . '/' . $avatar_filename)) && ($mode == 'editavatar') )
	{
...
function user_avatar_url($mode, &$error, &$error_msg, $avatar_filename)
{
	...
	return ( $mode == 'editavatar' ) ? "user_avatar = '" . str_replace("\'", "''", $avatar_filename) . "', user_avatar_type = " . USER_AVATAR_REMOTE : '';
...
function user_avatar_upload($mode, $avatar_mode, &$current_avatar, &$current_type, &$error, &$error_msg, $avatar_filename, $avatar_realname, $avatar_filesize, $avatar_filetype)
{
	...
	if ( $avatar_mode == 'remote' && preg_match('/^(http:\/\/)?([\w\-\.]+)\:?([0-9]*)\/([^ \?&=\#\"\n\r\t<]*?(\.(jpg|jpeg|gif|png)))$/', $avatar_filename, $url_ary) )
	{
		...
		if ( !$error && $avatar_filesize > 0 && $avatar_filesize < $board_config['avatar_filesize'] )
		{
			...
			$tmp_path = ( !@$ini_val('safe_mode') ) ? '/tmp' : './forum/' . $board_config['avatar_path'] . '/tmp';
	...
	if ( $width > 0 && $height > 0 )
	{
		...
		if ( $mode == 'editavatar' && $current_type == USER_AVATAR_UPLOAD && $current_avatar != '' )
		{
		...
		{
			@copy($tmp_filename, './forum/' . $board_config['avatar_path'] . "/$new_filename");
			@unlink($tmp_filename);
		}
		else
		{
			...
			$move_file($avatar_filename, 'forum/' . $board_config['avatar_path'] . "/$new_filename");
		}

		@chmod('./forum/' . $board_config['avatar_path'] . "/$new_filename", 0777);
		if ($width > $board_config['avatar_max_width'] || $height > $board_config['avatar_max_height'])
		{
			...
			$img_old = $imagecreatefrom_function ('./forum/' . $board_config['avatar_path'] . "/$new_filename");
			$img_new = imagecreatetruecolor ($width, $height);
			imagecopyresampled ($img_new, $img_old, 0, 0, 0, 0, $width, $height, $width_old, $height_old);
			$image_function ($img_new, './forum/' . $board_config['avatar_path'] . "/$new_filename");
			imagedestroy ($img_new);
		}

		$avatar_sql = ( $mode == 'editavatar' ) ? " user_avatar = '$new_filename', user_avatar_type = " . USER_AVATAR_UPLOAD : "'$new_filename', " . USER_AVATAR_UPLOAD;
	}
	else
	{

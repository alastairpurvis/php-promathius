...
function show_coppa()
{
	...
	$template->assign_vars(array(
		...
		"U_AGREE_OVER13" => append_sid("ucp.$phpEx?mode=register&amp;agreed=true"),
		"U_AGREE_UNDER13" => append_sid("ucp.$phpEx?mode=register&amp;agreed=true&amp;coppa=true"))
	);
	...
	$mode == 'register' )
{
	...
	{
		message_die_ext(GENERAL_ERROR, 'Could not select default dateformat', '', __LINE__, __FILE__, $sql);
	}
	...
	$user_avatar = ( empty($user_avatar_local) && $mode == 'editaccount' ) ? $userdata['user_avatar'] : '';
	$user_avatar_type = ( empty($user_avatar_local) && $mode == 'editaccount' ) ? $userdata['user_avatar_type'] : '';
...
{
	message_die_ext(GENERAL_MESSAGE, $lang['Username_taken'], '', __LINE__, __FILE__);
}
...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	if ($board_config['enable_confirm'] && $mode == 'register')
	{
		...
		else
		{
			...
			if (!($result = $db->sql_query($sql)))
			{
				message_die_ext(GENERAL_ERROR, 'Could not obtain confirmation code', '', __LINE__, __FILE__, $sql);
			}
			...
			if ($row = $db->sql_fetchrow($result))
			{
				...
				else
				{
					...
					if (!$db->sql_query($sql))
					{
						message_die_ext(GENERAL_ERROR, 'Could not delete confirmation code', '', __LINE__, __FILE__, $sql);
					}
				}
			}
			else
	...
	if ( !empty($new_password) && !empty($password_confirm) )
	{
		...
		else
		{
			if ( $mode == 'editaccount' )
			{
				...
				if ( !($result = $db->sql_query($sql)) )
				{
					message_die_ext(GENERAL_ERROR, 'Could not obtain user_password information', '', __LINE__, __FILE__, $sql);
				}
	...
	if ( isset($HTTP_POST_VARS['avatardel']) && $mode == 'editaccount' )
	{
	...
	if ( !$error )
	{
		if ( $avatar_sql == '' )
		{
			$avatar_sql = ( $mode == 'editaccount' ) ? '' : "'', " . USER_AVATAR_NONE;
		}

		if ( $mode == 'editaccount' )
		{
			if ( $email != $userdata['user_email'] && $board_config['require_once_activation'] != USER_ACTIVATION_NONE && $userdata['user_level'] != ADMIN )
			{
				$user_active = 0;
...
define('PROMATHIUS', true);
            ...
            mysql_safe_query("UPDATE ".$config['prefixes'][1]."_players SET username='$username' WHERE num='$userdata[empire_id]';");
			if ( !($result = $db->sql_query($sql)) )
			{
				message_die_ext(GENERAL_ERROR, 'Could not update users table', '', __LINE__, __FILE__, $sql);
			}
			...
			if ( !$user_active )
			{
				...
				$emailer = new emailer($board_config['smtp_delivery']);
 				...
 				else if ( $board_config['require_once_activation'] == USER_ACTIVATION_ADMIN )
 				{
 					...
 					if ( !($result = $db->sql_query($sql)) )
 					{
 						message_die_ext(GENERAL_ERROR, 'Could not select Administrators', '', __LINE__, __FILE__, $sql);
 					}
			...
			{
				$message = $lang['Profile_updated'] . '<br /><br />' . sprintf($lang['Click_return_cp'],  '<a href="' . append_sid("ucp.$phpEx?mode=editaccount") . '">', '</a>');
			}

			$template->assign_vars(array(
				"META" => '<meta http-equiv="refresh" content="5;url=' . append_sid("ucp.$phpEx?mode=editaccount") . '">')
			);
		...
		else
		{
			...
			if ( !($result = $db->sql_query($sql)) )
			{
				message_die_ext(GENERAL_ERROR, 'Could not obtain next user_id information', '', __LINE__, __FILE__, $sql);
			}
			...
			if ( $board_config['require_once_activation'] == USER_ACTIVATION_SELF || $board_config['require_once_activation'] == USER_ACTIVATION_ADMIN || $coppa )
			{
				$user_actkey = gen_rand_string(true);
				$key_len = 54 - (strlen($server_url));
				$key_len = ( $key_len > 6 ) ? $key_len : 6;
			...
			{
				message_die_ext(GENERAL_ERROR, 'Could not insert data into users table', '', __LINE__, __FILE__, $sql);
			}
			...
			if ( !($result = $db->sql_query($sql)) )
			{
				message_die_ext(GENERAL_ERROR, 'Could not insert data into groups table', '', __LINE__, __FILE__, $sql);
			}
			...
			{
				message_die_ext(GENERAL_ERROR, 'Could not insert data into user_group table', '', __LINE__, __FILE__, $sql);
			}
			...
			else if ( $board_config['require_once_activation'] == USER_ACTIVATION_SELF )
			{
...
else if ( $mode == 'editaccount' && !isset($HTTP_POST_VARS['avatargallery']) && !isset($HTTP_POST_VARS['submitavatar']) && !isset($HTTP_POST_VARS['cancelavatar']) )
{
...
else
{
	...
	$template->set_filenames(array(
		'body' => '../../../data/templates/ucp/profile_overview.tpl')
	);
	...
	if (!empty($board_config['enable_confirm']) && $mode == 'register')
	{
		...
		if (!($result = $db->sql_query($sql)))
		{
			message_die_ext(GENERAL_ERROR, 'Could not select session data', '', __LINE__, __FILE__, $sql);
		}
		...
		if ($row = $db->sql_fetchrow($result))
		{
			...
			if (!$db->sql_query($sql))
			{
				message_die_ext(GENERAL_ERROR, 'Could not delete stale confirm data', '', __LINE__, __FILE__, $sql);
			}
		}
		$db->sql_freeresult($result);
		...
		if (!($result = $db->sql_query($sql)))
		{
			message_die_ext(GENERAL_ERROR, 'Could not obtain confirm code count', '', __LINE__, __FILE__, $sql);
		}
		...
		if (!$db->sql_query($sql))
		{
			message_die_ext(GENERAL_ERROR, 'Could not insert new confirm code information', '', __LINE__, __FILE__, $sql);
		}

		unset($code);
		
		$confirm_image = '<img src="' . append_sid("ucp.$phpEx?mode=confirm&amp;id=$confirm_id") . '" alt="" title="" />';
		$s_hidden_fields .= '<input type="hidden" name="confirm_id" value="' . $confirm_id . '" />';

		$template->assign_block_vars('switch_confirm', array());
	}
	
	// Retrieve the player ranking
	$rank = @mysql_fetch_array(mysql_query("SELECT rank FROM game_players WHERE num='$userdata[empire_id]';"));
	
	if($userdata['is_empire'] == 0)
		$template->assign_block_vars('switch_noempire', array());
	if($userdata['is_empire'] == 1)
		$template->assign_block_vars('switch_empire', array());
	...
	$template->assign_vars(array(
		...
		'EMPIRE_NAME'	=>	$userdata['empire'],
		'EMPIRE_RANK'	=>	$rank['rank'],
		'EMPIRES_RULED'	=>	$userdata['empires_owned'],
		'EMPIRES_ABANDONED'	=>	$userdata['empires_abandoned'],
		'MAX_GREATNESS'	=>	round($userdata['max_greatness'], $config['greatness_roundoff']),
		'RATING'	=>	round($userdata['rating'], 2),
		...
		'U_SEARCH_USER' => append_sid("forum/search.$phpEx?search_author=" . $userdata['user_id']),

		'L_CURRENT_PASSWORD' => $lang['Current_password'],
		'L_NEW_PASSWORD' => ( $mode == 'register' ) ? $lang['Password'] : $lang['New_password'],
		'L_CONFIRM_PASSWORD' => $lang['Confirm_password'],
		'L_CONFIRM_PASSWORD_EXPLAIN' => ( $mode == 'editaccount' ) ? $lang['Confirm_password_explain'] : '',
		'L_PASSWORD_IF_CHANGED' => ( $mode == 'editaccount' ) ? $lang['password_if_changed'] : '',
		'L_PASSWORD_CONFIRM_IF_CHANGED' => ( $mode == 'editaccount' ) ? $lang['password_confirm_if_changed'] : '',
		...
		'L_ITEMS_require_onceD' => $lang['Items_require_onced'],
		...
		'S_PROFILE_ACTION' => append_sid("ucp.$phpEx"))
	);

...
$error = FALSE;
$error_msg = '';
$page_title = ( $mode == 'editprefs' ) ? $lang['Edit_profile'] : $lang['Register'];
	...
	$mode == 'register' )
{
	...
	else
	{
		...
		$showempire = ( isset($HTTP_POST_VARS['showempire']) ) ? ( ($HTTP_POST_VARS['showempire']) ? TRUE : 0 ) : $userdata['user_showempire'];
	}
	...
	$user_avatar = ( empty($user_avatar_local) && $mode == 'editprefs' ) ? $userdata['user_avatar'] : '';
	$user_avatar_type = ( empty($user_avatar_local) && $mode == 'editprefs' ) ? $userdata['user_avatar_type'] : '';
...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	if ( $signature != '' )
	{
		...
		$signature = prepare_message($signature, $allowhtml, $allowbbcode, $allowsmilies, $showempire, $signature_bbcode_uid);
	}
	...
	if ( isset($HTTP_POST_VARS['avatardel']) && $mode == 'editprefs' )
	{
	...
	if ( !$error )
	{
		if ( $avatar_sql == '' )
		{
			$avatar_sql = ( $mode == 'editprefs' ) ? '' : "'', " . USER_AVATAR_NONE;
		}
		...
		if ( $mode == 'editprefs' )
		{
			...
			$sql = "UPDATE " . USERS_TABLE . "
				SET user_viewemail = $viewemail, user_attachsig = $attachsig, user_allowsmile = $allowsmilies, user_showempire = $showempire, user_allowhtml = $allowhtml, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_notify_pm = $notifypm, user_popup_pm = $popup_pm, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_active = $user_active
			...
			{
				$message = $lang['Profile_updated'] . '<br /><br />' . sprintf($lang['Click_return_cp'],  '<a href="' . append_sid("ucp.$phpEx?mode=editprefs") . '">', '</a>');
			}

			$template->assign_vars(array(
				"META" => '<meta http-equiv="refresh" content="5;url=' . append_sid("ucp.$phpEx?mode=editprefs") . '">')
			);
...
else if ( $mode == 'editprefs' && !isset($HTTP_POST_VARS['avatargallery']) && !isset($HTTP_POST_VARS['submitavatar']) && !isset($HTTP_POST_VARS['cancelavatar']) )
{
	...
	$showempire = $userdata['user_showempire'];
...
if( isset($HTTP_POST_VARS['avatargallery']) && !$error )
{
	...
	display_avatar_gallery($mode, $avatar_category, $user_id, $email, $current_email, $coppa, $username, $email, $new_password, $cur_password, $password_confirm, $icq, $aim, $msn, $yim, $website, $location, $occupation, $interests, $signature, $viewemail, $notifypm, $popup_pm, $notifyreply, $attachsig, $allowhtml, $allowbbcode, $allowsmilies, $showempire, $allowviewonline, $user_style, $user_lang, $user_timezone, $user_dateformat, $userdata['session_id']);
}
else
{
	...
	$template->set_filenames(array(
		'body' => '../../../data/templates/ucp/profile_prefs.tpl')
	);
	...
	$template->assign_vars(array(
		...
		'ALWAYS_SHOWEMPIRE_YES' => ( $showempire ) ? 'checked="checked"' : '',
		'ALWAYS_SHOWEMPIRE_NO' => ( !$showempire ) ? 'checked="checked"' : '',
		...
		'L_CONFIRM_PASSWORD_EXPLAIN' => ( $mode == 'editprefs' ) ? $lang['Confirm_password_explain'] : '',
		'L_PASSWORD_IF_CHANGED' => ( $mode == 'editprefs' ) ? $lang['password_if_changed'] : '',
		'L_PASSWORD_CONFIRM_IF_CHANGED' => ( $mode == 'editprefs' ) ? $lang['password_confirm_if_changed'] : '',

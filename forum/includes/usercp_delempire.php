...
$error = FALSE;
$error_msg = '';
$page_title = ( $mode == 'delempire' ) ? $lang['Edit_profile'] : $lang['Register'];
	...
	$mode == 'register' )
{
	...
	$user_avatar = ( empty($user_avatar_local) && $mode == 'delempire' ) ? $userdata['user_avatar'] : '';
	$user_avatar_type = ( empty($user_avatar_local) && $mode == 'delempire' ) ? $userdata['user_avatar_type'] : '';
...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	if ( isset($HTTP_POST_VARS['avatardel']) && $mode == 'delempire' )
	{
	...
	if ( !$error )
	{
		if ( $avatar_sql == '' )
		{
			$avatar_sql = ( $mode == 'delempire' ) ? '' : "'', " . USER_AVATAR_NONE;
		}
...
define('PROMATHIUS', true);
			...
			{
				$message = $lang['Empire_deleted'] . '<br /><br />' . sprintf($lang['Click_return_cp'],  '<a href="' . append_sid("ucp.$phpEx") . '">', '</a>');
			}

			$template->assign_vars(array(
				"META" => '<meta http-equiv="refresh" content="5;url=' . append_sid("ucp.$phpEx?mode=delempire") . '">')
			);

			message_die_ext3(GENERAL_MESSAGE, $message);
		}
		else
		{
...
else if ( $mode == 'delempire' && !isset($HTTP_POST_VARS['avatargallery']) && !isset($HTTP_POST_VARS['submitavatar']) && !isset($HTTP_POST_VARS['cancelavatar']) )
{
...
else
{
	...
	$template->set_filenames(array(
		'body' => '../../../data/templates/ucp/profile_delempire.tpl')
	);
	...
	$template->assign_vars(array(
		...
		'L_CONFIRM_PASSWORD_EXPLAIN' => ( $mode == 'delempire' ) ? $lang['Confirm_password_explain'] : '',
		'L_PASSWORD_IF_CHANGED' => ( $mode == 'delempire' ) ? $lang['password_if_changed'] : '',
		'L_PASSWORD_CONFIRM_IF_CHANGED' => ( $mode == 'delempire' ) ? $lang['password_confirm_if_changed'] : '',
		...
		'L_ABANDON_DELAY' => $config['abandon_delay'],
		'L_AIM' => $lang['AIM'],

...
if (!defined('MINIFY_CACHE_DIR')) {
  ...
  define('MINIFY_CACHE_DIR', MINIFY_BASE_DIR . '/' . $scripts_cache);
}
...
class Minify {
  ...
  function handleRequest() {
    ...
    // requested.
    $type = preg_match('/\.jsc$/iD', $files[0]) ? TYPE_JS : TYPE_CSS;
  ...
  function _minify($string, $type = TYPE_JS) {
    return $type === TYPE_JS ? Minify::compressJS($string) :
        Minify::compressCSS($string);
  }
  ...
  function combine($css) {
	global $styles_cache, $style_dir, $data_root_path;
	$minify = true;
    ...
    if (MINIFY_USE_CACHE) {
		if($css)
		{
			$cacheFile = MINIFY_BASE_DIR . '/' . $styles_cache.'/styles_'.$this->getHash().'.css';
		}
		else
		{
			$cacheFile = MINIFY_CACHE_DIR.'/scripts_'.$this->getHash().'.js';
		}
	if($combined){
	...
	if($css)
		$combine = '/styles_'.$this->getHash().'.css';
	else
		$combine = '/scripts_'.$this->getHash().'.js';
    return $combine;
  }

...
$unum = $user[num];
$news = mysql_safe_query("SELECT * FROM $newsdb WHERE (clan1=$user[clan] OR clan2=$user[clan]) AND code=502 AND time>$time_limit AND id2!=$unum 
...
while ($new = mysql_fetch_array($news)) {
	...
	if($id1 == $user[num])
		...
		$name = sqlsafeeval("SELECT empire FROM $playerdb WHERE num=$id1;").' <a class=proflink href=?profile&target='.$id2.$authstr.'>(#'.$id1.')</a>';

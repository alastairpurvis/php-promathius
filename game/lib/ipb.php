...
function intForumNews() {
	...
	@mysql_select_db($dbname);
        ...
        $topics = @mysql_safe_query("SELECT tid,title,description,posts FROM ".$pr."_topics WHERE forum_id=$id ORDER BY start_date DESC LIMIT 0,$lim;");
...
//		$comments = $topic['posts'] - 1;
		...
		if($comments == 1)
			...
			$ctext = $comments.' comments';
                $post = @mysql_fetch_array(@mysql_safe_query("SELECT author_id,author_name,post_date,post FROM ".$pr."_posts WHERE topic_id=$id ORDER BY post_date ASC LIMIT 0,1;"));
                $body = $post['post'];

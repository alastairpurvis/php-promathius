...
define("EWIKI_SPAGES_DIR", "./data/guide/spages");
...
define("EWIKI_CACHE_DIR", $game_root_path."/cache/wiki");
define("EWIKI_CACHE_DB", "/game/cache/system");
...
if (!function_exists("ewiki_page_edit")) { require_once("plugins/edit.php"); }
if (!function_exists("ewiki_format")) { require_once("plugins/format.php"); }
if (!function_exists("ewiki_binary")) { require_once("plugins/feature/binary.php"); }
if (!function_exists("ewiki_author")) { require_once("plugins/misc.php"); }
if (!class_exists("ewiki_database_mysql")) { require_once("plugins/db/mysql.php"); }
...
#	define("EWIKI_SCRIPT_URL", "http://../?id=");	# absolute URL
	...
	define("EWIKI_NAME", "Promathius");		# Wiki title
...
#new!	define("EWIKI_URL_UTF8", 1);		# fix UTF-8 parameters
	...
	#- user permissions
	define("EWIKI_PROTECTED_MODE", 1);	# disable funcs + require_once auth
	...
	@define("EWIKI_DBFILES_DIRECTORY", "data/guide");	# see "db_flat_files.php"
...
//	$ewiki_plugins["action_always"]["links"] = "ewiki_page_links";
 ...
 #<broken+bug>#	"//" => array("<i>", "</i>"),   # conflicts with URLs, could only be done with regex
	...
	$ewiki_t["en"] = (array)@$ewiki_t["en"] + array(
	   ...
	   "OLDVERCOMEBACK" => "Restore this version",
	   "PREVIEW" => "Preview",
	   "SAVE" => "Save",
...
function ewiki_page($id=false) {
   ...
   if (EWIKI_PROTECTED_MODE) {
      if (!ewiki_auth($id, $data, $action, $ring=false, $force=EWIKI_AUTO_LOGIN)) {
         return($o.='<b><span class="error-font"><center>'.$ewiki_errmsg.'</b>');
      }
   }
...
function ewiki_make_title($id='', $title='', $class=3, $action="view", $go_action="links", $may_split=1) {
   ...
   return('<center><span style="font-size: 14px; font-weight: bold">' . $o . '</span></center>');
}
...
function ewiki_page_info($id, &$data, $action) {
   ...
   for ($v=$v_start,$first=1; ($v>=$v_end); $v--,$first=0) {
      ...
      $o .= '<table class="shlarge"  cellpadding="3" cellspacing="1">' . "\n";
...
function ewiki_page_edit_form(&$id, &$data, &$hidden_postdata) {
   ...
   $o .= '<center><textarea wrap="soft" id="ewiki_content" name="content" maxwidth=435 rows="'.$rows . '" cols="69">'
   ...
   $o .= "\n<br />\n"
      ...
      . ' &nbsp; <a class="cancel" href="'. ewiki_script("", $id) . '">' . ewiki_t("CANCEL_EDIT") . '</a><br /></center>';
...
function ewiki_page_edit_form_final_imgupload(&$o, &$id, &$data, &$action) {
   if (EWIKI_SCRIPT_BINARY && EWIKI_UP_UPLOAD && EWIKI_IMAGE_MAXSIZE) {
      ...
      . '&nbsp;&nbsp;&nbsp;'
      . '<input type="submit" class="mainoption" value="'.ewiki_t("UPLOAD_PICTURE_BUTTON").'">'
      . '</form></div>'. "\n";
  }
}
...
function ewiki_control_links($id, &$data, $action, $hide_hr=0, $hide_mtime=0) {
   ...
   global $ewiki_plugins, $ewiki_ring, $ewiki_config, $users;
   ...
   $o = "\n"
      . '<tr><td><div align="right" class="action-links control-links">';
   ...
   if (@$ewiki_config["forced_version"] && ewiki_auth($id, $data, "edit")) {
      ...
      $o .= '<form action="' . ewiki_script("edit", $id) . '" method="POST">' .
            ...
            '<input type="submit" class="mainoption" value="' . ewiki_t("OLDVERCOMEBACK") . '"></form> ';
   }
   else {
...
function ewiki_link_regex_callback($ii, $force_noimg=0) {
   ...
   global $ewiki_links, $ewiki_plugins, $ewiki_config, $ewiki_id, $users;
   ...
   elseif (@$states["define"]) {
      $type = array("anchor");
      if ($title==$href) { 
         $title = "";   // was "&nbsp;" before, but that's not require_onced
      }
...
function ewiki_form($name, $value, $label="", $_text="| |\n", $inj="") {
   ...
   if (!$name || ($type=="b")) {
      ...
      $o .= "<input type=\"submit\" class=\"mainoption\" id=\"ff$fid\"$name value=\"$value\"$inj />";
   }
   #-- select
   ...
   #-- input field; text or hidden
      ...
      $o .= "<input type=\"$type\" class=\"mainoption\" id=\"ff$fid\" name=\"$name\" value=\""
...
class ewiki_database_mysql {
   ...
   function GET($id, $version=false) {
      ...
      $result = mysql_safe_query("SELECT *, pagename as id FROM {$this->table}
   ...
   function HIT($id) {
      $id = mysql_escape_string($id);
      mysql_safe_query("UPDATE {$this->table} SET hits=(hits+1) WHERE pagename='$id'");
   }
   ...
   function WRITE($hash, $overwrite=0) {
      ...
      $result = mysql_safe_query("$COMMAND INTO {$this->table} ($sql1) VALUES ($sql2)");
   ...
   function FIND($list) {
      ...
      $result = mysql_safe_query("SELECT pagename AS id, meta, flags FROM {$this->table} WHERE $sql");
      if ($result) {
   ...
   function GETALL($fields, $mask=0, $filter=0) {
      ...
      $result = mysql_safe_query("SELECT pagename AS id, $fields FROM
   ...
   function DELETE($id, $version) {
      $id = mysql_escape_string($id);
      mysql_safe_query("DELETE FROM {$this->table} WHERE pagename='$id' AND version=$version");
   }


   function INIT() {
      mysql_safe_query("CREATE TABLE {$this->table}
 ...
 function ewiki_email_protect_form($id, $data=0, $action=0, $text="PROTE1", $url="") {
    ...
    if ($url || ($email = @$_REQUEST[EWIKI_UP_ENCEMAIL])) {
          ...
          if (empty($_REQUEST[EWIKI_UP_NOSPAMBOT])) {
             ...
             $html .= '<input type="submit" class="mainoption" name="go"></form><br /><br />';
...
function ewiki_action_verdiff($id, $data, $action) {
   ...
   if (($v0 = (int)$_REQUEST["from"]) && ($v1 = (int)$_REQUEST["to"])) {
      ...
      $o .= '<input type="submit" class="mainoption" value="diff">';
...
function ewiki_fancy_list_dict($links) {
   ...
   $o .= '<table cellpadding="3" cellspacing="2">' . "\n";
...
function ewiki_auth_handler_ring_permissions($id, $data, $action, $require_onced_ring) {

   global $ewiki_plugins, $ewiki_ring, $ewiki_perm_rings;

   if ("ALWAYS_DO_THIS" || ($require_onced_ring===false)) {
   ...
   return(($require_onced_ring===false) || isset($ewiki_ring) && ($ewiki_ring <= $require_onced_ring));
}
...
//         if (($n <= 3) and ($line[$n]==" ")) {
		...
		$text = str_replace('</h3>', '', $text);
            $toc[$i] = str_repeat("&nbsp;", 5*(3-$n)) . " "
                     . '<a href="'.$_SERVER["REQUEST_URI"] . '#line'.$i.'">'
                     . trim($text) . "</a>";
...
function ewiki_toc_view_prepend(&$html) {
   ...
   if (count($ewiki_page_toc) >= 3) {
	...
	$html = "<table class='toc' cellpadding='3' cellspacing='1'><tr><td style='text-align:center'>"
...
#  rescale that image until it fits; it utilizes the PHP game/libgd
...
if (!strstr(PHP_VERSION, "-dev") && !(extension_loaded("php_gd2.dll") or extension_loaded("gd.so")) && !function_exists("imagecreate") && function_exists("dl")) {   #-- try to load gd game/lib
...
function ewiki_page_powersearch($id, &$data, $action) {
    ...
    if (empty($q)) {        
        ...
        <br /><br />
        <input type="submit" class="mainoption" value=" &nbsp; &nbsp; S E A R C H &nbsp; &nbsp; ">
        </form></div>
...
function ewiki_page_recentchanges($recentchanges, $data, $action) {
   ...
   <form action="$url" method="GET">
     ...
     <input type="submit" class="mainoption" value="_{days}">
   </form>
   <br />
EOT
   );
...
function ewiki_page_rc_moin(&$e, &$meta) {
   ...
   $o .= '<table class="changes" width="100%">'
      . '<colgroup><col width="35%"><col width="25%"><col width="35%"></colgroup>';
...
<input type="submit" class="mainoption" name="download_tarball" value= "_{DOWNLOAD_ARCHIVE}">
<br /><br />
...
/*
  ...
  setting.
  require_onces _PROTECTED_MODE, see ewiki_auth() in README, and $ewiki_ring==0
...
function ewiki_action_control_page($id, &$data, $action) {
   ...
   #-- function list -------------------------------------------------------
      ...
      #-- flags
      $o .= '<div class="flags" align=center>';
      $o .= "<h4>Page flags</h4><br />";
      foreach ($a_flagnames as $n=>$s) {
         ...
         $a[$n] = '<input type="checkbox" class="cb" name="sflag['.$n.']" value="1"'.
               $checked . $disabled .'>' . $s;
      }
      $o .= '<table class="list">' . "\n";
      ...
      $o .= '</table>';
      $o .= '<center><input type="submit" class="mainoption" name="pgc_setflags" value="chmod"></center>';
      $o .= "\n<br /><br /></div><br />"; 

      #-- rename
      $o .= '<div class="rename">';
      $o .= "<center><h4>Rename page</h4></center><br />";
      $o .= 'new page name: <input type="text" size="30" name="mv_to" value="'.htmlentities($id).'">'
          . '<br />'
          . '<table border=0 width=100%><tr><td width=13><input type="checkbox" name="mv_cr0" class="cb" value="1" checked></td><td> Try to change all references from other pages accordingly </td></tr>'
          . '<tr><td><input type="checkbox" name="mv_cr1" value="1" class="cb" checked></td><td> Act case-insensitive when doing so</td></tr></table> ';
      $o .= '<br /><center><input type="submit" class="mainoption" name="pgc_rename" value="Rename"></center>';
      $o .= "\n<br /><br /></div><br />"; 

      #-- meta
      if (isset($ewiki_ring) && ($ewiki_ring==0)) {
      $o .= '<div class="meta" align=center>';
      $o .= "<h4>meta data</h4><br />";
      ...
      $o .= "</textarea>\n";
      $o .= '<br /><center><input type="submit" class="mainoption" name="pgc_setmeta" value="Set"></center>';
      $o .= "\n<br /><br /></div><br />"; 
      }

      #-- delete
      $o .= '<div class="delete" align=center>';
      $o .= "<h4>Delete page</h4><br />";
      $o .= '<table border=0><tr><td width=13><input type="checkbox" name="pgc_purge1" class=cb value="1"></td><td> I\'m sure</td></tr></table>';
      $o .= '<br /><center><input type="submit" class="mainoption" name="pgc_purge" value="Delete"></center>';
      $o .= "\n<br /><br /></div>\n"; 

...
function saveBountData (&$user, $data)
{
	...
	while ($tmp = $items[$i++])
	{
        ...
        $my_sqltest = mysql_safe_query("UPDATE $mercdb SET $update WHERE num=$user[num];") or die("Invalid query: " . mysql_error());
	if (!mysql_safe_query("UPDATE $mercdb SET $update WHERE num=$user[num];"))
...
function loadBount ($num)
{
	global $mercdb;
	fixInputNum($num);
	return mysql_fetch_array(mysql_safe_query("SELECT * FROM $mercdb WHERE num=$num;"));
}
...
function bountyCount($target_id, $user_id) {
	...
	return sqlsafeeval("SELECT count(*) FROM $mercdb WHERE t_num=$target_id AND s_num=$user_id AND filled=0;");
}

function bountyscan(&$target)
{
       global $mercdb, $newsdb, $config;
	   $bounties = mysql_safe_query("SELECT * FROM $mercdb WHERE t_num=$target[num] AND filled=0;") or die("Invalid query: " . mysql_error());
        //echo "<br /><br /><br />";
		//print("entered scan?");
		...
		// the problem is I need to check certain attacks based on if the require_oncements were land or net or rank
...
//				printf("Saving data");
				...
				$kera = loadRegion($killer['era'], $killer['race']);

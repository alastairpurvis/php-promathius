...
function getHierarchies() {
	...
	mysql_safe_query("UPDATE $playerdb SET map_show=-1;");
	mysql_safe_query("UPDATE $clandb SET map_show=-1;");

	$total = 0;
	$hierarchies = array();
	$clans = mysql_safe_query("SELECT num FROM $clandb WHERE members>0;");
	$clanplayers = array();
	while($clan = mysql_fetch_array($clans)) {
		$net = sqlsafeeval("SELECT SUM($factor) FROM $playerdb WHERE clan=$clan[num] AND land>0 AND disabled<2;");
		$hierarchies["c$clan[num]"] = array(	num=>$clan[num],
							net=>$net,
							type=>1);
		$total += $net;
		$players = mysql_safe_query("SELECT num, clan, $factor FROM $playerdb WHERE clan=$clan[num] AND land>0 AND disabled<2;");
	...
	$players = mysql_safe_query("SELECT num, clan, $factor FROM $playerdb WHERE clan=0 AND land>0 AND disabled<2;");
	while($player = mysql_fetch_array($players)) {

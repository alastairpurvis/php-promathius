...
function getSellCosts ($type, $num='') {
	...
	$costbonus = 1 + (1 - $config[mktshop]) * (($users[barracks]*$config['buildingoutput']) / $users[land]) + $config[mktshop] * ((getBuildings('income_total', $users)*$config['buildingoutput']) / $users[land]);
...
function sellUnits ($type, $num='') {
	...
	if ($amount == 0)
		return;
	elseif ($amount < 0)
		$msg_error1 = "Cannot disband a negative amount of $uera[$ts].";
	elseif ($amount > $cansell[$ts])
		$msg_error1 = "You can not disband that many $uera[$ts].";
	else
	{
		$users[cash] += $cost;
		if($type == 'troop') {
		...
		if($type != 'food' && $type != 'runes')
		{
			if(gamefactor($amount) == 1)
				$itemname = $uera[$ts.'alt'].' was';
			else
				$itemname = $uera[$ts].' were';
		}
		else
			$itemname = strtolower($uera[$ts]);
		$msg[] = commas(gamefactor($amount))." $itemname disbanded and you were refunded with ".commas(gamefactor($cost))." gold";
	}
}

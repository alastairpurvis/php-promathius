...
if(isset($mod_rewrite) and $mod_rewrite) $urlp="{$main_url}/{$forum}_"; else $urlp="{$indexphp}action=vtopic&amp;forum=$forum&amp;sortBy=$sortBy&amp;page=";
...
$topic_reverse='';
$topic_views=$cols[8];
if(isset($themeDesc) and in_array($topic,$themeDesc)) $topic_reverse="<img src=\"{$main_url}/images/topic_reverse.gif\" align=middle border=0 alt=\"\">&nbsp;";
...
$pageNavCell=pageNav(0,$numReplies,$urlp,$viewmaxreplys,TRUE);
...
$nTop=1;
//OR (isset($regUsrForums) and in_array($forum, $regUsrForums) and $user_id==0)
if (((isset($poForums) and in_array($forum, $poForums) and $isMod!=1) OR (isset($roForums) and in_array($forum, $roForums) and $isMod!=1) OR (isset($regUsrForums) and in_array($forum, $regUsrForums) and !$user_id)) and $user_id!=1) {
$main=preg_replace("/(<form.*<\/form>)/Uis", '', $main);
$nTop=0;
$newTopicLink='';
}

if (((isset($poForums) and in_array($forum, $poForums) and $isMod!=1) OR (isset($roForums) and in_array($forum, $roForums) and $isMod!=1)) and $user_id!=1) {
$main=preg_replace("/(<form.*<\/form>)/Uis", '', $main);
$nTop=0;
$newTopicLink='';
}

...
$key_vemail=$dbUserSheme['user_viewemail'][0];
$key_email=$dbUserSheme['user_email'][0];
$key_web=$dbUserSheme['user_website'][0];

if ($row=db_simpleSelect(0,$Tu,'*',$dbUserId,'=',$user)) {
if ($row[$key_vemail]!=1) $row[$key_email]=$usEmail; else $row[$key_email]='<a href="mailto:'.$row[$key_email].'">'.$row[$key_email].'</a>';
if ($row[$key_web]!='') $row[$key_web]='<a href="'.$row[$key_web].'" target="_blank">'.$row[$key_web].'</a>'; else $row[$key_web]='';
$row[$dbUserDateKey]=convert_date($row[$dbUserDateKey]);

...
$c='';
if(!in_array(1,$mods)) $mods[]=1;
$GLOBALS['xtr']=getClForums($mods,'where','',$GLOBALS['dbUserId'],'or','=');
if ($row=db_simpleSelect(0,$GLOBALS['Tu'],$GLOBALS['dbUserSheme']['username'][1].','.$GLOBALS['dbUserId'],'','','',$GLOBALS['dbUserId'])){
do $c.="<a href=\"{$GLOBALS['indexphp']}action=userinfo&amp;user={$row[1]}\">{$row[0]}</a>, "; while ($row=db_simpleSelect(1));
}
$GLOBALS['xtr']='';
...
$numPosts=db_simpleSelect(2,$Tf,'SUM(posts_count)');

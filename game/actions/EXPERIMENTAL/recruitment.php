...
if ($do_changetax)
{
	fixInputNum($new_tax);
	if ($new_tax < 5)
		endScript("Cannot set your tax that low!");
	if ($new_tax > 65)
		endScript("Cannot set your tax that high!");
	$users[tax] = $new_tax;
...
if ($do_changeindustry)
{
	...
	// thus...
	if($total > 100)
		endScript("Output cannot be set that high. <Br>The total must amount to 100 or less.");

	// if we got here okay, we can save
	saveUserData($users,"production");
	$tpl->assign('printmessage', 'Recruitment settings updated.');
}
if ($do_changepass)
{

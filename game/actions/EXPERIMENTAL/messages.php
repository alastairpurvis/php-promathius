...
initGUI();
include($game_root_path."/lib/msgcron.php");
...
function message ($msg_body, $msg_title, $msg_dest, $msg_replyto) {
	global $time, $users, $messagedb, $playerdb, $allclan, $search, $replace;
	...
	$output = '';
		...
		if($allclan) {
				if($users[msgcred] < 5)
					endScript("You need at least 5 message credits. Please wait a few minutes.");

				$warlords = mysql_safe_query("SELECT num,empire FROM $playerdb WHERE clan=$users[clan] AND num!=$users[num] AND land>0 AND disabled !=2 AND disabled != 3 ORDER by empire DESC;");
				$msg_title .= " (To Clan)";
			...
			if ($users[msgcred] == 0 && empty($msg_replyto))
				endScript("You have run out of credits. Please wait a few minutes.");

			if (($msg_replyto)) {
				fixInputNum($msg_replyto);
				mysql_safe_query("UPDATE $messagedb SET replied=1 WHERE id=$msg_replyto;");
			}
			...
			$name = sqlsafeeval("SELECT empire FROM $playerdb WHERE num=$msg_dest_num;");
...
if ($lockdb)
	endScript("Messaging is currently disabled!");
...
{
	if(empty($msg_dest))
		endScript("Please specify the recipient of your message!");
...
if ($do_delete) {
	fixInputNum($msg_id);
	mysql_safe_query("UPDATE $messagedb SET deleted=1 WHERE deleted=0 AND id=$msg_id AND dest=$users[num];");
	mysql_safe_query("UPDATE $messagedb SET deleted=3 WHERE deleted=2 AND id=$msg_id AND dest=$users[num];");
...
if ($do_deleteall && $php_block!="No")
{
	...
	mysql_safe_query("UPDATE $messagedb SET deleted=1 WHERE deleted=0 AND dest=$users[num];");
	mysql_safe_query("UPDATE $messagedb SET deleted=3 WHERE deleted=2 AND dest=$users[num];");
	print "All messages deleted!<hr>\n";
}

if ($do_delete_read) {
	mysql_safe_query("UPDATE $messagedb SET deleted=1 WHERE deleted=0 AND dest=$users[num] AND readd=1;");
	mysql_safe_query("UPDATE $messagedb SET deleted=3 WHERE deleted=2 AND dest=$users[num] AND readd=1;");
...
if ($do_delete_selected)
{
	...
	//echo "i = $i - $boxes[$i] = boxes[i]";
		fixInputNum($boxes[$i]);
		$msg = mysql_safe_query("SELECT * FROM $messagedb WHERE id=$boxes[$i];");
		$message = mysql_fetch_array($msg);
		if($users[num] != $message[dest]) endScript("You can't delete another person's messages.");
		mysql_safe_query("UPDATE $messagedb SET deleted=1 WHERE deleted=0 AND dest=$users[num] AND id=$boxes[$i];");
		mysql_safe_query("UPDATE $messagedb SET deleted=3 WHERE deleted=2 AND dest=$users[num] AND id=$boxes[$i];");
		$i++;
...
if ($view!="")
{
  ...
  fixInputNum($view);
  $msg = mysql_safe_query("SELECT * FROM $messagedb WHERE id=$view;");

  $vmessage = mysql_fetch_array($msg);
  if($users[num] != $vmessage[dest]) endScript("That's not your message.");
 ...
 // $vmessage[msg] = wordwrap($vmessage[msg], 120, "<br />", 1);
  //print_r($vmessage);
  mysql_safe_query("UPDATE $messagedb SET readd=1 WHERE id=$vmessage[id];");
...
$msgs = mysql_safe_query("SELECT * FROM $messagedb WHERE dest=$users[num] AND (deleted=0 OR deleted=2) ORDER BY $order_by $asc;");

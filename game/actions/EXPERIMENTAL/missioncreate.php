...
$uclan = loadClan($users[clan]);
	...
	if($users[clan])
	        ...
	        $warquery = "SELECT num, empire, land, disabled, clan FROM $playerdb WHERE land>0 AND disabled != 3 AND disabled != 2 AND num != $users[num] ORDER BY rank";
...
if($do_set)
{
	...
	if(!empty($_POST['net_drop'])) fixInputDouble($net_drop);
	fixInputNum($targ);
	fixInputNum($all);
	fixInputNum($anon);
	...
	if(!isset($targ))
	{
				$mis_error=$config['error']['mission_target'];
	mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
	...
	if(!$target[num])
	{
					$mis_error=$config['error']['mission_targetnotemp'];
	mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
	$uri = urldecode($_SERVER['REQUEST_URI']);
	$action = substr($uri, strpos($uri, '?') + 1);
	header("Location: ?" . $action); 
	...
	if($target[land] < 1 || $target[disabled] ==2 || $target[disabled] == 3)
	{
						$mis_error=$config['error']['mission_targetimp'];
	mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
	$uri = urldecode($_SERVER['REQUEST_URI']);
	$action = substr($uri, strpos($uri, '?') + 1);
	header("Location: ?" . $action); 
	...
	if(bountyCount($target[num], $users[num]) > 0)
	{
							$mis_error=$config['error']['mission_limit'];
	mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
	$uri = urldecode($_SERVER['REQUEST_URI']);
	$action = substr($uri, strpos($uri, '?') + 1);
	header("Location: ?" . $action); 
	...
	if($users[num_bounties] == $max)
	{
								$mis_error=$config['error']['mission_nomore'];
	mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
	$uri = urldecode($_SERVER['REQUEST_URI']);
	$action = substr($uri, strpos($uri, '?') + 1);
	header("Location: ?" . $action); 
	...
	if ($target[clan] != 0 && $tar_clan != 0 && $users[clan]!=0 && ($uclan[ally1] == $target[clan]) || ($uclan[ally2] == $target[clan]) || ($uclan[ally3] == $target[clan]) || ($uclan[ally4] == $target[clan]) || ($uclan[ally5] == $target[clan]))
		if($target[clan]!=0)
			{
													$mis_error=$config['error']['mission_ally'];
	mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
	$uri = urldecode($_SERVER['REQUEST_URI']);
	$action = substr($uri, strpos($uri, '?') + 1);
	header("Location: ?" . $action); 
	...
	{
											$mis_error=$config['error']['mission_notenough'];
	mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
	$uri = urldecode($_SERVER['REQUEST_URI']);
	$action = substr($uri, strpos($uri, '?') + 1);
	header("Location: ?" . $action); 
	...
	{
												$mis_error=$config['error']['targetclan'];
	mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
	$uri = urldecode($_SERVER['REQUEST_URI']);
	$action = substr($uri, strpos($uri, '?') + 1);
	header("Location: ?" . $action); 
	...
	foreach($config[troop] as $num => $mktcost) {
		...
		{
														$mis_error=$config['error']['mission_toomanytroops'];
	mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
	$uri = urldecode($_SERVER['REQUEST_URI']);
	$action = substr($uri, strpos($uri, '?') + 1);
	header("Location: ?" . $action); 
	endScript();
		...
		if($troop[$num] < 0)
				{
														$mis_error=$config['error']['mission_negamount'];
	mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
	$uri = urldecode($_SERVER['REQUEST_URI']);
	$action = substr($uri, strpos($uri, '?') + 1);
	header("Location: ?" . $action); 
	endScript();
		...
		if($troop[$num] > $cansend[$num])
				{
														$mis_error=$config['error']['mission_notenoughtroops'];
	mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
	$uri = urldecode($_SERVER['REQUEST_URI']);
	$action = substr($uri, strpos($uri, '?') + 1);
	header("Location: ?" . $action); 
	...
	if($cash_give > $users[cash] || $rune_give > $users[runes] || $food_give > $users[food])
		{
	$mis_error=$config['error']['mission_notenoughresources'];
	mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
	$uri = urldecode($_SERVER['REQUEST_URI']);
	$action = substr($uri, strpos($uri, '?') + 1);
	header("Location: ?" . $action); 
	...
	if($givenothing)
		{
				$mis_error=$config['error']['mission_givenothing'];
		mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
		$uri = urldecode($_SERVER['REQUEST_URI']);
		$action = substr($uri, strpos($uri, '?') + 1);
		header("Location: ?" . $action); 
	...
	if($land_drop == -1 && $rank_drop == -1 && $net_drop == -1)
	{
				{
				$mis_error=$config['error']['mission_nocondition'];
		mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
		$uri = urldecode($_SERVER['REQUEST_URI']);
		$action = substr($uri, strpos($uri, '?') + 1);
		header("Location: ?" . $action); 
	...
	if($users[num] == $targ)
			{
				$mis_error=$config['error']['mission_targetself'];
		mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
		$uri = urldecode($_SERVER['REQUEST_URI']);
		$action = substr($uri, strpos($uri, '?') + 1);
		header("Location: ?" . $action); 
	...
	if(  ($all && $all_three_land > $target[land] && $all_three_rank < $target[rank] && $all_three_net > $target[networth]) || ( !$bounty[all_1] && ($just_one_land > $target[land] || $just_one_rank < $target[rank] || $just_one_net > $target[networth])))
					{
				$mis_error=$config['error']['mission_conditionmet'];
		mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
		$uri = urldecode($_SERVER['REQUEST_URI']);
		$action = substr($uri, strpos($uri, '?') + 1);
		header("Location: ?" . $action); 
	...
	$bountyset = '<span class="success-font">'.$config['lang']['mission_created1']. "<a class=proflink href=?profiles&num=$target[num]$authstr>$target[empire]</a>".$config['lang']['mission_created2'].'</span><br /><br />';
	//print $bountyset;
	...
	if($ubount[anon])
		...
		$current_value = sqlsafeeval("SELECT new_value FROM $headlinedb where name='new_missions';") + 1;
		if(sqlsafeeval("SELECT new_value FROM $headlinedb where name='new_missions';") != 0)
			$updated_strings = (sqlsafeeval("SELECT new_strings FROM $headlinedb where name='new_missions';")).'|'. $ubount[num];
		else
			$updated_strings = $ubount[num];
		mysql_safe_query("UPDATE $headlinedb SET new_value='$current_value', new_strings='$updated_strings' WHERE name='new_missions';");
}
...
if($_POST['edit']) {
	foreach($_POST as $key => $value) {
		if(substr($key, 0, 7) == "editid_") {
			...
			if($edit_bounty[filled] == 1 || $edit_bounty[s_num] != $users[num]) endScript("This bounty has already been fulfilled, or is not yours.");
...
if($_POST['do_add']) {
	...
	if($cash_give > $cansend[cash] || $food_give > $cansend[food] || $rune_give > $cansend[runes])
							{
				$mis_error="You cannot add that much.";
		mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
		$uri = urldecode($_SERVER['REQUEST_URI']);
		$action = substr($uri, strpos($uri, '?') + 1);
		header("Location: ?" . $action); 
	...
	foreach($config[troop] as $num => $mktcost) {
		if($troop[$num] < 0)
										{
				$mis_error="You cannot give a negative amount.";
		mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
		$uri = urldecode($_SERVER['REQUEST_URI']);
		$action = substr($uri, strpos($uri, '?') + 1);
		header("Location: ?" . $action); 
		...
		if($troop[$num] > $cansend[$num])
										{
				$mis_error="You can not add that much.";
		mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
		$uri = urldecode($_SERVER['REQUEST_URI']);
		$action = substr($uri, strpos($uri, '?') + 1);
		header("Location: ?" . $action); 
	...
	if($cash_give > $users[cash] || $rune_give > $users[runes] || $food_give > $users[food] || $troop0_give > $users[troop0] || $troop1_give > $users[troop1] || $troop2_give > $users[troop2] || $troop3_give > $users[troop3])
												{
				$mis_error="It would be unwise to offer a reward that you cannot provide.";
		mysql_query("UPDATE ".$config['prefixes'][1]."_players SET new_error='$mis_error' WHERE num='$users[num]';");
		$uri = urldecode($_SERVER['REQUEST_URI']);
		$action = substr($uri, strpos($uri, '?') + 1);
		header("Location: ?" . $action); 
	...
	if($cash_give < 0 || $rune_give < 0 || $food_give < 0 || $troop0_give < 0 || $troop1_give < 0 || $troop2_give < 0 || $troop3_give < 0)
		...
		if($edit_bounty[filled] == 1 || $edit_bounty[s_num] != $users[num]) endScript("This mission has already been completed or i not yours.");
		//if($edit_bounty[edits] > $max_edits-1) endScript("You have already edited this bounty the max amount.");

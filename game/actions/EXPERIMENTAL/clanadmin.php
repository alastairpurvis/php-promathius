...
<option value="0"<?php if ($uclan[$item] == 0) print " selected";
	...
	?>>None</option>
<?php $list = mysql_safe_query("SELECT num,name,tag FROM $clandb WHERE members>0 ORDER BY num DESC;");
...
if ($do_removeempire) {
	...
	endScript("<b>$enemy[empire] <a class=proflink href=?profiles&num=$enemy[num]$authstr>(#$enemy[num])</a></b> has been removed from $uclan[name].");
} 
if ($do_changepass) {
...
if ($do_changerelations) {
	...
	endScript("Clan relations changed.");
} 
/*
if ($do_createclan) {
?> THIS IS A TEST <?PHP
...
<tr><th>Modify?</th>
    <th>Empire</th></tr>
<?php
$dblist = mysql_safe_query("SELECT empire,num FROM $playerdb WHERE clan=$uclan[num];");

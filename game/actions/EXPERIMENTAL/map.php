...
require_once($game_root_path."/lib/military.php");
require_once($game_root_path."/lib/map.php");
...
function divStart($other, $type) {
	...
	if($type == 'p') {
		...
		if($color == "good")
			$color = "dead";
		$name = $other[empire].($other[online]?'*':'').' <a class="proflinkdk" href="?profiles&num='.$other[num].$authstr.'">(#'.$other[num].')</a>';
		$mapbuffer .= '<div class="'.$class.'" style="'.$style.'">'."\n".'<img width='.$width.' height='.$height.' border=0 src="data/images/m'.$color.'.jpg" />'."\n</div>\n";
		...
		$class = "mapC";
		$mapbuffer .= '<div class="'.$class.'" style="'.$style.'">'."\n".'<img width='.$width.' height='.$height.' border=1 src="data/images/spacer.gif" />'."\n</div>\n";
	}
}
...
function LloadClan($id) {
	...
	fixInputNum($id);
	$data = mysql_fetch_array(mysql_safe_query("SELECT * FROM $clandb WHERE num=$id;"));
...
foreach($player_lookup_table as $id => $entry) {
	fixInputNum($id);
	$data = mysql_fetch_array(mysql_safe_query("SELECT * FROM $playerdb WHERE num=$id;"));
	foreach($entry as $var => $value)
		$data[$var] = $value;
	divStart($data, 'p');
}

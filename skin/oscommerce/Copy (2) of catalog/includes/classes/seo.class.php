 ...
 */	
	function SEO_URL_INSTALLER(){
		...
		$this->default_config['SEO_ADD_CPATH_TO_PRODUCT_URLS'] = array('DEFAULT' => 'false',
		   															   'QUERY' => "INSERT INTO `".TABLE_CONFIGURATION."` VALUES ('', 'Add cPath to product URLs?', 'SEO_ADD_CPATH_TO_PRODUCT_URLS', 'false', 'This setting will append the cPath to the end of product URLs (i.e. - some-product-p-1.php?cPath=xx).', GROUP_INSERT_ID, ".$x.", NOW(), NOW(), NULL, 'tep_cfg_select_option(array(''true'', ''false''),')"
																		);
		$x++;
		$this->default_config['SEO_ADD_CAT_PARENT'] = array('DEFAULT' => 'true',
		   													'QUERY' => "INSERT INTO `".TABLE_CONFIGURATION."` VALUES ('', 'Add category parent to begining of URLs?', 'SEO_ADD_CAT_PARENT', 'true', 'This setting will add the category parent name to the beginning of the category URLs (i.e. - parent-category-c-1.php).', GROUP_INSERT_ID, ".$x.", NOW(), NOW(), NULL, 'tep_cfg_select_option(array(''true'', ''false''),')"
															);
		$x++;
				...
				case 'products_id':
					...
					switch(true){
						case ( $page == FILENAME_PRODUCT_INFO && !$this->is_attribute_string($p2[1]) ):
							$url = $this->make_url($page, $this->get_product_name($p2[1]), $p2[0], $p2[1], '.php', $separator);
                            $this->ValidateName($url, "p", $p2[1], $connection, $separator);
							break;
						case ( $page == FILENAME_PRODUCT_REVIEWS ):
							$url = $this->make_url($page, $this->get_product_name($p2[1]), 'products_id_review', $p2[1], '.php', $separator);
                            $this->ValidateName($url, "pr", $p2[1], $connection, $separator);
							break;
						case ( $page == FILENAME_PRODUCT_REVIEWS_INFO ):
							$url = $this->make_url($page, $this->get_product_name($p2[1]), 'products_id_review_info', $p2[1], '.php', $separator);
				...
				case 'cPath':
					switch(true){
						case ($page == FILENAME_DEFAULT):
							$url = $this->make_url($page, $this->get_category_name($p2[1]), $p2[0], $p2[1], '.php', $separator);
				...
				case 'manufacturers_id':
					switch(true){
						case ($page == FILENAME_DEFAULT && !$this->is_cPath_string($params) && !$this->is_product_string($params) ):
							$url = $this->make_url($page, $this->get_manufacturer_name($p2[1]), $p2[0], $p2[1], '.php', $separator);
				...
				case 'authors_id':
					switch(true){
						case ($page == FILENAME_ARTICLES):
							$url = $this->make_url($page, $this->get_authors_name($p2[1]), $p2[0], $p2[1], '.php', $separator);
...
//////////--------EOF AUTHORS
				case 'tPath':
					switch(true){
						case ($page == FILENAME_ARTICLES):
							$url = $this->make_url($page, $this->get_topic_name($p2[1]), $p2[0], $p2[1], '.php', $separator);
				...
				case 'lPath':
					switch(true){
						case ($page == FILENAME_LINKS):
							$url = $this->make_url($page, $this->get_link_name($p2[1]), $p2[0], $p2[1], '.php', $separator);
				...
				case 'articles_id':
					switch(true){
						case ($page == FILENAME_ARTICLE_INFO):
							$url = $this->make_url($page, $this->get_article_name($p2[1]), $p2[0], $p2[1], '.php', $separator);
				...
				case 'info_id':
					switch(true){
						case ($page == FILENAME_INFORMATION):
							$url = $this->make_url($page, $this->get_information_name($p2[1]), $p2[0], $p2[1], '.php', $separator);
				...
				case 'faqdesk_id':
                                        switch(true){
                                                case ($page == FILENAME_FAQDESK_INFO):
                                                        $url = $this->make_url($page, $this->get_faqdesk_name($p2[1]), $p2[0], $p2[1], '.php', $separator);
                                                        break;
												case ($page == FILENAME_FAQDESK_REVIEWS_INFO):
                                                        $url = $this->make_url($page, $this->get_faqdesk_name($p2[1]), 'faqdesk_reviews_id', $p2[1], '.php', $separator);
                                                        break;
												case ($page == FILENAME_FAQDESK_REVIEWS_ARTICLE):
                                                        $url = $this->make_url($page, $this->get_faqdesk_name($p2[1]), 'faqdesk_article_id', $p2[1], '.php', $separator);
                                                        break;
                                                default: 
                                                        $container[$p2[0]] = $p2[1];
                                                        break;
				...
				case 'faqPath':
                                        switch(true){
                                                case ($page == FILENAME_FAQDESK_INDEX):
                                                        $url = $this->make_url($page, $this->get_faqdesk_categories_name($p2[1]), $p2[0], $p2[1], '.php', $separator);
                                                        break;
                                                default: 
                                                        $container[$p2[0]] = $p2[1];
                                                        break;
				...
				case 'pages_id':
					switch(true){
						case ($page == FILENAME_PAGES):
							$url = $this->make_url($page, $this->get_pages_name($p2[1]), $p2[0], $p2[1], '.php', $separator);
							break;
						default: 
							$container[$p2[0]] = $p2[1];
							break;
...
case 'pollid':
                                        switch(true){
                                                case ($page == 'pollbooth.php'):
                                                        $url = $this->make_url($page, $this->get_polls_name($p2[1]), $p2[0], $p2[1], '.php', $separator);
                                                        break;
                                                default: 
                                                        $container[$p2[0]] = $p2[1];
                                                        break;
				...
				case 'newsdesk_id':
                                        switch(true){
                                                case ($page == FILENAME_NEWSDESK_INFO):
                                                        $url = $this->make_url($page, $this->get_newsdesk_name($p2[1]), $p2[0], $p2[1], '.php', $separator);
                                                        break;
												case ($page == FILENAME_NEWSDESK_REVIEWS_INFO):
                                                        $url = $this->make_url($page, $this->get_newsdesk_name($p2[1]), 'newsdesk_reviews_id', $p2[1], '.php', $separator);
                                                        break;
												case ($page == FILENAME_NEWSDESK_REVIEWS_ARTICLE):
                                                        $url = $this->make_url($page, $this->get_newsdesk_name($p2[1]), 'newsdesk_article_id', $p2[1], '.php', $separator);
                                                        break;
                                                default: 
                                                        $container[$p2[0]] = $p2[1];
                                                        break;
				...
				case 'newsPath':
                                        switch(true){
                                                case ($page == FILENAME_NEWSDESK_INDEX):
                                                        $url = $this->make_url($page, $this->get_newsdesk_categories_name($p2[1]), $p2[0], $p2[1], '.php', $separator);
                                                        break;
                                                default: 
                                                        $container[$p2[0]] = $p2[1];
                                                        break;
 ...
 * @return string
 */	
	function make_url($page, $string, $anchor_type, $id, $extension = '.php', &$separator){
 ...
 */	
  ...
  { 
    ...
    {
      if (($pos = strpos($parts[count($parts) - 1], ".php")) !== FALSE)
       $id = substr($parts[count($parts) - 1], 0, $pos);       //strip .php 
  ...
  { 
    ...
    if (($pos = strpos($current_page, "?sid")) !== FALSE)

...
<?php
  ...
  $col = 0;
    ...
    $info_box_contents[$row][$col] = array('align' => 'center',
                                           'params' => 'Width="33%" valign="top"',
                                           'text' => $border . '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products['products_id']) . '">' . tep_image(DIR_WS_IMAGES . $image, $products['products_name'], PRODUCT_IMAGE_WIDTH, PRODUCT_IMAGE_WIDTH) . '</a>' . '<br><br><a class="inactive" href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products['products_id']) . '">'. (PRODUCT_LIST_NAME ? '' . $products['products_name'] . '' : '' ) . '</a><br>' . ($products['short_desc'] && (PRODUCT_SHORT_DESC == 'true') ? $products['short_desc'] . '<br>' : '') . $show_price . $borderend . $button);
